...
tryAcquire 顾名思义，就是**尝试获取锁**，AQS在这里没有对其进行功能的实现，只有一个抛出异常的语句，我们需要自己对其进行实现，可以对其重写实现公平锁、不公平锁、可重入锁、不可重入锁
...
1. 自旋enq 方法将并发添加节点的请求通过CAS跟自旋将尾节点的添加变得`串行化`起来。说白了就是让节点放到正确的队尾位置。
