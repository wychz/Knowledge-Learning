...
![image-20210329232149086](../../../AppData/Roaming/Typora/typora-user-images/image-20210329232149086.png)
...
1. 业务逻辑问题：
   ...
   2. 接着用top -H -p pid来找到 CPU 使用率比较高的一些线程，然后将占用最高的 pid 转换为 16 进制 printf '%x\n' pid 得到 nid：
...
1. 降低开发者测试的粒度。因为普通的用例都是针对微服务接口的，这里面包括了一些非常大型的微服务。然后需要设计了一种基于方法测试的方案，这部分是我来做的。
...
2. 另外一个就是Hbase内存方案的设计。Hbase有RowKey，列族，列限定符这些概念，这里就手动设计了一个内存方案的HBase，将所有的操作转换到一个嵌套的TreeMap上面，走我们的方法。比如key是一个行键，value是一个列族的列表，然后每个列族又包含map，key是列名，value是列值这样。然后所有的操作都会转换为对map的操作。因为测试嘛，数据量不大，所以可以实现的。最后用一个反射的方式，将实际使用的类替换成我们的代理类
...
非侵入式负荷分解是我导师实验室跟国家电网合作的一个科研项目，主要是深度学习相关的。主要的内容是通过从所有电器总功率里分解出每台电器的功率。比如说，我在家里使用电脑，台灯，空调， 冰箱，洗衣机这些电器。我只要采集到在某一时刻整个家庭所有电器的总功率，就能从其中知道你在用哪些电器，这些电器的功率是多少。
