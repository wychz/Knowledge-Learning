...
LOW LEVER TEST， 通常由开发人员实施，用来测试语句的覆盖率。LLT是在开发阶段对系统进行尽早、尽快的验证，发现的缺陷也更容易定位和解决。我们编写测试用例时只需要引入jar包，无需其他任何额外环境，就可以进行测试的编写。

### 1. 普通测试遇到的挑战

1. 中间件准备投入大，像是数据库，Redis，kafka这些环境准备，准备测试数据和运行实例，都需要大量的时间投入。这就导致在某些场景下，环境的数据的准备远超过测试本身的价值。
2. 测试的可读性，可维护性差。这个是主要是当我们写完开发者测试用例之后，最后展现在看板上的，只有测试覆盖率这些指标。但是用例本身的可读性和可维护性就相对较低。后续当我们的代码发生改变或者逻辑改变，我们想要看或者维护以前的测试用例就出现了问题。

### 2. 问题解决方案

1. 提供了包括Redis，Kafka，数据库，HBase这些业务中可能用到的In-memory方案，实际测试的时候，我们不需要去准备开发环境了，框架已经全部集成进来。在框架里利用mock或者反射的方式，将所有对生产环境的使用，全部转移到这些本地的内存化方案中。
2. 对于这些可维护性差的问题，我们框架提供了一种能力，让用例基本上全部以自然语言的方式书写，目的就是为了让任何人，包括看不懂代码的这部分人都能知道我们在测什么。我们只需提供要测试的接口url，请求方式，请求参数，就可以执行用例并输出测试结果，检查是否和期望结果相匹配。写代码的部分的话，就是在写测试用例的过程中，开发者只需要限定测试的边界。比如说我们待测的微服务还去请求了其他微服务的接口，只需要在这里用mock的方式，构造一个期待的返回结果即可。
3. 整个框架的启动流程就是启动tomcat，然后tomcat去进行像spring，hibernate还有各类中间件和本地微服务的启动，还有用例的自动执行。

### 3. 我的工作

1. 降低开发者测试的粒度。因为普通的用例都是针对微服务接口的，这里面包括了一些非常大型的微服务。然后需要设计了一种基于方法测试的方案，这部分是我来做的。

   当时觉得也没多难，就是想指定待测方法，参数这些，然后将请求转发到我自己的解析逻辑上，做一些json字符串的参数解析，用一些开源的Gson，fastjson这些工具进行映射，然后反射调用一下实际方法去做这样。结果这种方案完全不可行。很多类都是我们平时手动去写一个类，就是公司这边为了解耦和方便修改的需求吧，这边的类和接口很多都是通过写yang文件或者yaml文件，然后配合一些内部插件去自动编译生成的。传参的方式也不一样，很多场景都是通过建造者模式去生成参数的。这就导致我们实际传参需要传对应的builder对象，由框架通过反射去生成一个实际的参数。这就不能通过fastjson，Gson这些工具去实现自动转换，需要我来手动的对参数进行数据解析，调整和最终生成。这个过程中还会遇到像是用builder生成的参数里面还yaml模型，yang模型参数，这种一层层循环嵌套，yaml，yang模型出现在map，list的泛型里，这种类似很多复杂的场景。

   然后你也知道，json是没有类型信息嘛，转换参数的时候可能遇到需要转换的成员变量参数类型为接口，就没有办法直接转换了，然后这里也是参考了一下这边leader的意见，就去做一个handler，手动指定接口与实现类的对应关系，然后在每一层解析的时候遇到接口参数都会尝试做一个类型替换。然后种种因素吧，这中间就涉及非常多的处理过程。

   然后为了简化使用，尽可能减少用户的配置，尽量吧所有流程能自动化的地方都自动化了，比如遇到重载方法的一些自动选择，参数构造的方式选择，建造者模式的自动适配这些。

   此外还做了一些用例之间的数据隔离。理论上所有的测试用例应该是互不影响的，但是实际上前一个用例在数据库存储的数据会影响到第二个用例。所以设计了一些数据隔离方案，可以开发者自动选择是否开启数据隔离，数据初始化，数据恢复等。这里是用一个开源的工具dbutils去做的。原理的话没有细看，大概就是利用数据库的像是binlog，undolog 去做一个数据的全量备份或者回滚操作。

2. 另外一个就是Hbase内存方案的设计。Hbase有RowKey，列族，列限定符这些概念，这里就手动设计了一个内存方案的HBase，将所有的操作转换到一个嵌套的TreeMap上面。最后用一个反射的方式，将实际使用的类替换成我们的代理类，走我们的方法。比如key是一个行键，value是一个列族的列表，然后每个列族又包含map，key是列名，value是列值这样。然后所有的操作都会转换为对map的操作。因为测试嘛，数据量不大，所以可以实现的。
...
客户端这边我定义了一个RpcReference注解。当扫描到标准了这个注解的类的时候，框架会自动构造一个代理类，用这个代理类去替换掉原本的bean。这个代理类采用了JDK动态代理的方式，执行方法的时候，自动构造我们的RpcRequest。然后通过指定的负载均衡方式，查找到对应的服务器节点，通过netty的handler构造一个RpcMessage，向指定服务器发送请求，并返回响应。
...
**TCP沾包拆包**主要是通过自定义协议实现的。我们在网络中请求的传输，是通过序列化的方式传输的。我们自己定义了一个RpcMessage类完成的。这个类里面包含了我们的消息类型（心跳，消息），编码方式，压缩方式，请求id，请求data等。我们在用encoder编码的时候，会去计算序列化和压缩之后消息体的长度，与消息头拼接求出总长度，然后写在消息头里，decode的时候根据这个长度进行解码，拆分出一个一个包。
...
**SPI机制**：就是我们先会去META-INF.extensions目录下去找要实现的接口的全限定类名对应的文件，找对应接口的实现类，加载到本地缓存里。这里主要就是进行一些字符串的解析，和反射创建实现类这些工作，下次取的时候，根据名字直接从本地缓存里进行取对应的类实例。
