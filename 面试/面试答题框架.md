...
1. 介绍：LOW LEVER TEST， 通常由开发人员实施，来保障语句的覆盖率。LLT是在开发阶段对系统进行尽早、尽快的验证，发现的缺陷也更容易定位和解决。底层基于karate，公司的llt对karate源码进行了修改，并重新进行了封装，集成了多种开发环境，让测试随处都可以开发。Karate是一种不需要掌握任何的Java知识也可以写出测试的代码，让任何人都能读懂这个测试在干什么。即使是非编程人员，测试代码也很容易编写。我们测试时只需要引入jar包，无需其他任何额外环境，就可以进行测试的编写。我主要做了一些像是数据隔离，参数解析，插件集成（像是内存级hbase的开发），还有一些边界mock的工作。
...
框架的整个流程是这样的，我定义了两个注解，客户端的@RpcReference，和服务端的@RpcService。然后重写了Springbean的postProceessBeforeInitialization方法和postProcessAfterInitialization方法。Spring框架在初始化bean的时候，会对标记了RpcService的注解进行服务发布，发布到zookeeper上去。标记了RpcReference的方法，进行一个bean的替换，通过反射的方式让这个bean的代理类去换掉这个bean。这样实际调用方法的时候，会调用这个代理类的方法去向远程客户端发起请求，并返回结果，实现客户端对远程方法的调用。然后序列化采用了json和protobuf的方式，json的话根据实际情况，比如方法参数是否有接口这些情况，拼接实现类参数，然后服务端会进行额外解析这样。

为了防止请求丢失，在本地缓存了所有已发出但是没有接收到响应的请求，用requestId和具体的请求相对应。没有收到响应的话，就会在下次发送请求前进行重新发送，收到响应就从内部去除掉这条缓存。

一致性hash我这里先求所有服务器的md5码，然后哈希算法求出值。使用treemap保存。选择的时候就先tailMap找出所有大于等于hash值的服务器，然后firstEntry第一台服务器的地址。



TCP沾包拆包主要是通过自定义协议实现的，我们在用encoder编码的时候，会去计算序列化和压缩之后消息体的长度，与消息头拼接求出总长度，然后写在消息头里，decode的时候根据这个长度进行解码
...
开发embeddedHbase，就是一个基于内存的hbase。

这里主要是通过cglib动态代理实现的。像是HConnection，HTable，HBase全部动态代理转到自己的构建的本地的实现类上。然后将所有的对数据库的操作，转换为对本地Map的操作，这里缓存主要是用TreeMap去实现的。因为HBase有行键，列族，列这些属性，这里我们全部映射到一个Map上。比如key是一个行键，value是一个列族的列表，然后每个列族又包含map，key是列名，value是列值这样。

[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-H6q4CzuY-1617355644920)(img/llt/v2-22e7f861a6a079a558a1fb173dbd8b9e_720w.jpg)]

然后所有的操作都会转换为对map的操作。因为测试嘛，数据量不大，所以可以实现的。
