...
LOW LEVER TEST， 通常由开发人员实施，来保障语句的覆盖率。LLT是在开发阶段对系统进行尽早、尽快的验证，发现的缺陷也更容易定位和解决。我们编写测试用例时只需要引入jar包，无需其他任何额外环境，就可以进行测试的编写。

### 1. 普通开发者测试框架遇到的挑战

1. 中间件准备投入大，像是数据库，Redis，kafka这些环境准备，准备测试数据和运行实例，都需要大量的时间投入。这就导致在某些场景下，环境的数据的准备远超过测试本身的价值。
2. 测试的可读性，可维护性差。这个是主要是当我们写完开发者测试用例之后，最后展现在看板上的，只有测试覆盖率这些指标。但是用例本身的可读性和可维护性就相对较低。后续当我们的代码发生改变或者逻辑改变，我们想要维护以前的测试用例就出现了问题。

### 2. 问题解决方案

1. 提供了包括Redis，Kafka，数据库，HBase这些业务中可能用到的In-memory方案，实际测试的时候，我们不需要去准备开发环境了，框架已经全部集成寄来。在框架里利用mock或者反射的方式，将所有对生产环境的使用，全部转移到这些本地的内存化方案中。
