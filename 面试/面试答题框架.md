...
2. Collections：集合框架提供的工具类，包含很多操作集合的静态方法，排序，搜索，线程安全等。
   1. `addAll(), sort(), swap(), max(), min(), replaceAll()`
...
1. 类型：
   ...
   7. **G1(Garbage First)收集器** **(标记-整理算法)**： **Java堆并行收集器**：G1回收的范围是整个Java堆(包括新生代，老年代)
      ...
      2. 三色标记算法：
         ...
         * 可能存在的问题：并发标记阶段，多线程同时对对象进行修改，出现白对象漏标情况。如Mutator赋予一个黑对象该白对象的引用，然后Mutator删除了灰对象到白对象的直接或间接引用。
...
1. **对象头**：12B，包括对象标记和类型指针。
   ...
   ![图片](https://mmbiz.qpic.cn/mmbiz_png/wJvXicD0z2dUETXrwicYZsnZw1F1Uzl11Tib2M829boSZg3Udib1yzNCdxDkjYh7zpRGRgjFEw9SjgnMUV3PYgz5Ww/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
[详细](https://www.jianshu.com/p/cf57726e77f2)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/wJvXicD0z2dUETXrwicYZsnZw1F1Uzl11TMNILD90nTdMwyuVAhwFzq0aice1o7WwJBichIXY26nNK3yicDE613eYOw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
1. 原理：
   1. 每个Java对象都有一个关联的**monitor**，JVM会根据环境找到对象的monitor，根据monitor的状态进行加解锁的判断。synchronized的对象锁，锁标识位为10，其中指针指向的是`monitor`对象（也称为管程或监视器锁）的起始地址。

      1. `monitor`是由`ObjectMonitor`实现的，其主要数据结构如下
         ...
         ![图片](https://mmbiz.qpic.cn/mmbiz_png/wJvXicD0z2dUETXrwicYZsnZw1F1Uzl11TlPcKu97GWaDHmEs3LccHZ4bgQsrBuibjcicq8qGRPkBJGOQC7D3ELAqQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

   2. 同步代码块

      使用**monitorenter和monitorexit**这两个字节码指令获取和释放monitor。加锁锁的计数器加一，减为0锁释放。这两个字节码指令都需要一个引用类型的参数指明要**加锁和解锁的对象**。对于**同步普通方法**， 锁是当前实例对象；对于**静态同步方法**，锁是当前类的class对象；对于**同步方法块**，锁是synchronized里的对象

   3. 同步方法：

      没有通过指令monitorenter和monitorexit来完成（理论上其实也可以通过这两条指令来实现），不过相对于普通方法，**其常量池中多了ACC_SYNCHRONIZED标示符**。JVM就是根据该标示符来实现方法的同步的：当方法调用时，调用指令将会检查方法的 ACC_SYNCHRONIZED 访问标志是否被设置，如果设置了，执行线程将先获取monitor，获取成功之后才能执行方法体，方法执行完后再释放monitor。在方法执行期间，**其他任何线程都无法再获得同一个monitor对象**。 其实本质上没有区别，只是方法的同步是一种隐式的方式来实现，无需通过字节码来完成。

   4. [参考1](https://www.pianshen.com/article/40891696663/)， [参考2](https://www.jianshu.com/p/e624460c645c)
...
3. 使用：
   1. 修饰实例方法：`synchronized`关键词作用在方法的前面，用来锁定方法，其实默认锁定的是`this`对象。
   2. 修饰静态方法：`synchronized`还是修饰在方法上，不过修饰的是静态方法，等价于锁定的是`Class`对象，
   3. 修饰代码块：函数体内部对于要修改的参数区间用`synchronized`来修饰，相比与锁定函数这个范围更小，可以指定锁定什么对象。
4. 锁开销大的原因：
   
   1. 因为监视器锁（monitor）是依赖于底层的操作系统的`Mutex Lock`来实现的，而操作系统实现线程之间的切换时需要从`用户态转换到核心态`，这个状态之间的转换需要相对比较长的时间，时间成本相对较高，这也是早期的`synchronized`效率低的原因。
...
1.  定义：即时编译器对检测到不可能存在共享数据竞争的锁进行消除。消除锁是虚拟机另外一种锁的优化，这种优化更彻底，Java虚拟机在JIT编译时通过对运行上下文的扫描，去除不可能存在共享资源竞争的锁，通过这种方式消，除没有必要的锁，可以节省毫无意义的请求锁时间，我们知道`StringBuffer`是线程安全的，里面包含锁的存在，但是如果我们在函数内部使用`StringBuffer`那么代码会在JIT后会自动将锁释放掉哦。
...
3. 一旦有其他线程尝试获取锁，偏向模式立即结束。如线程2要竞争锁对象，而偏向锁不会主动释放因此还是存储的线程1的threadID），那么需要查看Java对象头中记录的线程1`是否存活`，如果没有存活，那么锁对象被重置为无锁状态，其它线程（线程2）可以竞争将其设置为偏向锁；如果存活，那么立刻查找该线程（线程1）的栈帧信息，如果还是需要继续持有这个锁对象，那么暂停当前线程1，撤销偏向锁，升级为 `轻量级锁`，如果线程1不再使用该锁对象，那么将锁对象状态设为无锁状态，重新偏向新的线程。
...
3. 自旋：线程2的CAS失败，那么线程2就尝试使用自旋锁来等待线程1释放锁。自旋锁简单来说就是让线程2`在循环中不断CAS尝试获得锁对象`。但是如果自旋的**「时间太长」**也不行，因为自旋是要消耗CPU的，因此**「自旋的次数是有限制」**的，比如10次或者100次，如果自旋次数到了线程1还没有释放锁，或者线程1还在执行，线程2还在自旋等待，那么这个时候轻量级锁就会膨胀为重量级锁。重量级锁把除了拥有锁的线程都阻塞，防止CPU空转。
4. 解锁：解锁同样通过CAS进行。如果对象markword仍然指向线程的锁记录，就用CAS把对象的markword和线程复制的markword交换回来。替换成功同步完成，失败则说明有其他的线程尝试获取过锁，就要在释放锁的同时唤醒被挂起的线程。
...
5. 流程：
   ...
   1. **AQS**内部维护着一个**FIFO**的队列，即**CLH**队列，提供先来先服务的公平性。**AQS**的同步机制就是依靠**CLH**队列实现的。**CLH**队列是**FIFO**的双端双向链表队列(方便尾部节点插入)。线程通过**AQS**获取锁失败，就会将线程封装成一个**Node**节点，通过**CAS**原子操作插入队列尾。当有线程释放锁时，会尝试让队头的next节点占用锁
...
1. **独占模式**：锁只会被一个线程占用。
2. 获取：`acquire`方法，维护一个同步队列，使用`tryAcquire`方法安全地获取线程同步状态，获取失败的线程会被构造同步节点并通过`addWaiter`方法加入同步队列尾部，在队列中自旋。之后调用`acquireQueue`方法使该节点以死循环的方式获取同步状态，如果获取不到则阻塞，被阻塞线程的唤醒主要依靠前驱结点的出队或被中断实现。移出队列或停止自旋的条件是前驱结点是头结点且成功获取了同步状态。
3. 释放：`release`方法。调用`tryRelease`方法释放同步状态，然后调用`unparkSuccessor`唤醒头结点的后继节点，使后继节点重新尝试获取同步状态。

#### 2. 共享锁

1. **共享模式**：多个线程获取同一个锁可以成功，如ReadLock。
2. 获取：使用`acquireShared`方法，调用`tryAcquireShared`方法尝试获取同步状态，返回值为int类型。不小于0表示能够获取同步状态。
3. 释放：调用`releaseShared`方法，释放后会唤醒后续处于等待状态的节点。和独占式的区别是`tryReleaseShared`方法必须确保同步状态安全释放，通过循环CAS保障，因为释放同步状态的操作会来自多个线程。
...
3. 内存屏障：
   ...
   3. LoadLoad屏障，禁止volatile读与后面的普通读重排序
      ...
      1. 示例：Load1; LoadLoad; Load2
   
         该屏障确保Load1数据的装载先于Load2及其后所有装载指令的的操作
   
   4. LoadStore屏障，禁止volatile读和后面的普通写重排序
   
      1. 示例：Load1; LoadStore; Store2
   
         确保Load1的数据装载先于Store2及其后所有的存储指令刷新数据到内存的操作
4. 原理：用**volatile**变量修饰的共享变量进行写操作的时候会使用**CPU**提供的**Lock**前缀指令，在**CPU**级别的功能如下：
...
2. JVM退出时，守护线程中的finally块不一定执行，因为JVM中没有非守护线程时需要立即退出，所有守护线程立即终止。守护线程最典型的应用就是GC（垃圾回收器）。
...
1. sleep
   ...
   2. 锁：不释放锁
   3. 资源：不让出系统资源
   4. 一般指定睡眠时间。sleep方法相当于让线程进入阻塞状态
   5. 范围：在线程里调用
...
3. yield：当前线程让出CPU时间片给优先级相同或更高的线程，回到RUNNABLE状态。与其他线程一起竞争CPU时间片。不会释放锁
...
4. 使用：
   ...
   1. submit方法提交任务
   2. 任务分配，将任务封装成Task：
      1. 缓冲执行：将任务放到阻塞队列里面去，等待线程去取任务
      2. 直接执行，分配一个线程
      3. 任务拒绝，采用配置的不同的拒绝策略
   3. 任务获取：线程会被封装成为工作线程worker，去任务队列里面取任务执行。
...
6. 选择策略：
   ...
   1. 性质：
      1. CPU密集型：配置尽可能小的线程池。N+1个线程。N为CPU核心数
...
8. 线程池状态：
   ...
   2. **SHUTDOWN**（关闭）：不接受新任务，会处理队列任务。
      1. `shutdown`方法或关`shutdownNow`关闭线程池。
...
10. Worker线程管理
    ...
    2. 实现：**实现Runnable接口，持有一个线程thread，一个初始化的任务firstTask**。
       ...
       1. thread为调用构造方式时用ThreadFactory创建的线程，可以用来执行任务。
       2. firstTask用来保存传入的第一个任务，如果该任务非空，线程会在启动初期立即执行该任务，对应核心线程创建的情况，如果该值为null，需要去任务列表（workQueue）中取任务执行。

       ![](img/%E9%9D%A2%E8%AF%95%E7%AD%94%E9%A2%98%E6%A1%86%E6%9E%B6/%E7%BA%BF%E7%A8%8B%E6%B1%A0worker.jpg)

11. 线程池种类（使用Executors创建）：
    ...
    * 为什么不推荐用这种方式创建：
      * CachedThreadPool是一个根据需要创建新线程的线程池。对于非核心线程空闲60s时将被回收。因为Integer.MAX_VALUE非常大，可以认为是可以无限创建线程的，在资源有限的情况下容易引起OOM异常。
      * SingleThreadExecutor是单线程线程池，只有一个核心线程。当一个任务提交时，首先会创建一个核心线程来执行任务，如果超过核心线程的数量，将会放入队列中，因为LinkedBlockingQueue是长度为Integer.MAX_VALUE的队列，可以认为是无界队列，因此往队列中可以插入无限多的任务，在资源有限的时候容易引起OOM异常
      * FixedThreadPool是固定核心线程的线程池。和SingleThreadExecutor类似，由于使用的是LinkedBlockingQueue，在资源有限的时候容易引起OOM异常。
      * 总结：因为Excutors提供的创建线程池的方法，使用了默认的构造参数，容易导致OOM的错误，所以推荐TheadPoolExcutor()构造方法，自定义线程池，以便自己对创建的线程池有全面的了解，避免出现OOM的错误。

12. 使用场景：

    1. 快速响应用户请求：

       1. 用户发起的实时请求，服务追求响应时间。比如说用户要查看一个商品的信息，那么我们需要将商品维度的一系列信息如商品的价格、优惠、库存、图片等等聚合起来，展示给用户。

       2. 业务开发同学往往会选择使用线程池这种简单的方式，将调用封装成任务并行的执行，缩短总体响应时间。另外，使用线程池也是有考量的，这种场景最重要的就是获取最大的响应速度去满足用户，所以应该不设置队列去缓冲并发任务，调高corePoolSize和maxPoolSize去尽可能创造多的线程快速执行任务。

          ![图12 并行执行任务提升任务响应速度](https://p0.meituan.net/travelcube/e9a363c8577f211577e4962e9110cb0226733.png)

    2. 快速处理批量任务：

       1. 离线的大量计算任务，需要快速执行。比如说，统计某个报表，需要计算出全国各个门店中有哪些商品有某种属性，用于后续营销策略的分析，那么我们需要查询全国所有门店中的所有商品，并且记录具有某属性的商品，然后快速生成报表。

       2. 这种场景需要执行大量的任务，我们也会希望任务执行的越快越好。这种情况下，也应该使用多线程策略，并行计算。但与响应速度优先的场景区别在于，这类场景任务量巨大，并不需要瞬时的完成，而是关注如何使用有限的资源，尽可能在单位时间内处理更多的任务，也就是吞吐量优先的问题。所以应该设置队列去缓冲并发任务，调整合适的corePoolSize去设置处理任务的线程数。在这里，设置的线程数过多可能还会引发线程上下文切换频繁的问题，也会降低处理任务的速度，降低吞吐量。
...
线程的创建也分为`用户态线程`跟`内核态线程`。

1. 用户态线程：
   1. 定义：在用户空间实现的线程，由用户态的线程库来完成线程的管理。操作系统按进程维度进行调度，**当线程在用户态创建时应用程序在用户空间内要实现线程的创建、维护和调度。操作系统对线程的存在一无所知**！操作系统只能看到进程看不到线程。所有的线程都是在用户空间实现。在操作系统看来，每一个进程只有一个线程。
   2. 好处：
...
2. 内核态线程：
   1. 定义：在内核中实现的线程，是由内核管理的线程，线程对应的 TCB 在操作系统里，这样线程的创建、终止和管理都是由操作系统负责。内线程模式下一个用户线程对应一个内核线程。
   2. 好处：
...
3. 轻量级进程：**轻量级进程是内核支持的用户线程**，它是基于内核线程的高级抽象，系统只有先支持内核线程才能有 LWP。一个进程可有1~N个LWP，每个 LWP 是跟内核线程一对一映射的，也就是 LWP 都是由一个内核线程支持。**轻量级进程本质还是进程**，只是跟普通进程相比LWP跟其他进程共享大部分逻辑地址空间跟系统资源
4. 协程：协程 Coroutines 是一种比线程更加轻量级的微线程。类比一个进程可以拥有多个线程，一个线程也可以拥有多个协程。可以简单的把协程理解成子程序调用，每个子程序都可以在一个单独的协程内执行。**协程并没有增加线程数量，只是在线程的基础之上通过分时复用的方式运行多个协程**
...
[TCP IP1](https://mp.weixin.qq.com/s/mXLmI1tVFXtgGqQGtZRclg)      [TCP IP2](https://blog.csdn.net/qq_43412060/article/details/107115196)

### 4.HTTP协议

#### 1. 定义：

**超文本传输协议**的缩写，英文是Hyper Text Transfer Protocol。它是从WEB服务器传输超文本标记语言(HTML)到本地浏览器的传送协议。

#### 2. **原理**：

1. HTTP是一个基于TCP/IP通信协议来传递数据的协议，传输的数据类型为HTML 文件,、图片文件, 查询结果等。HTTP协议一般用于B/S架构（）。浏览器作为HTTP客户端通过URL向HTTP服务端即WEB服务器发送所有请求。
...
#### 5. 请求，响应：[详细](https://www.cnblogs.com/lauhp/p/8979393.html)
...
* Get和Post区别
  ...
  * REST服务：GET幂等，读同一个资源得到同一个数据。POST非幂等，每次对资源的改变不同。

    ​						GET不改变服务器资源，POST改变服务器资源

  * 请求参数：GET数据附加在URL之后，数据放在请求头中。？分割URL和数据，参数以&相连。

    ​						POST请求放在请求体中

  * 安全性：GET明文URL，POST包装到请求体，更安全

  * 请求大小：GET请求长度有限，发送数据较小。POST无限制。
...
- 3×× : 重定向，要完成请求必须进行进一步处理
  ...
  304 ： 已缓存
  - 301和302区别：
    - 301表示旧地址A已经被永久移除了（这个资源不可访问了）。搜索引擎在抓取新内容的同时也就旧地址交换为重定向之后网址。
    - 302表示旧地址A的资源还在（仍然可以访问），搜索引擎会抓取新的内容而保存旧的网址。
- 4×× : 客户端错误，请求不合法
...
3. 对称加密和非对称加密：
   ...
   3. 建议：对称加密加密消息，非对称加密加密秘钥
   
4. 摘要算法：摘要算法的主要特征是加密过程不需要密钥，并且经过加密的数据无法被解密。消息摘要算法主要应用在`数字签名`领域，作为对明文的摘要算法。著名的摘要算法有RSA公司的**MD5**算法和 **SHA-1** 算法及其大量的变体。
...
5. [通信步骤](https://blog.csdn.net/ustccw/article/details/76691248)：

   1. 客户端 ---URL---服务器，与服务器建立SSL链接

      1. Client Hello + 一个随机数  + 支持的加密算法
      2. Server Hello + 一个随机数 + 选择一个Client支持的加密算法

   2. 服务器收到请求，返回证书信息（有公钥）给客户端

   3. 客户端与服务器协商SSL安全等级（信息加密等级）。客户端根据双方同意的加密等级，建立会话秘钥，用网站的公钥加密秘钥，传送给网站。

      1. 客户端验证证书合法性，产生随机数字Pre-master，将preMaster用证书公钥加密发给服务器
      2. random1 + random2 + premaster + 加密算法 得到 共享的会话秘钥

   4. 服务器用私钥解密出会话秘钥

      1. 服务器用私钥解密得出Pre-MASTER
      2. random1 + random2 + premaster + 加密算法 得到 共享的会话秘钥。
...
2. 四次挥手
   ...
   4. 客户端：ACK=1，ack=N+1，客户端TIME_WAIT，服务器CLOSED

   * 详细：
     - 第一次挥手：Client发送一个FIN，用来关闭Client到Server的数据传送，Client进入FIN_WAIT_1状态。
     - 第二次挥手：Server收到FIN后，发送一个ACK给Client，确认序号为收到序号+1（与SYN相同，一个FIN占用一个序号），Server进入CLOSE_WAIT状态。
     - 第三次挥手：Server发送一个FIN，用来关闭Server到Client的数据传送，Server进入LAST_ACK状态。
     - 第四次挥手：Client收到FIN后，Client进入TIME_WAIT状态，接着发送一个ACK给Server，确认序号为收到序号+1，Server进入CLOSED状态，完成四次挥手。 注意：此时必须等待2MSL之后，客户端才会进入CLOSE状态。MSL为最长报文段寿命，即报文段的最长存活时间。
3. 三次握手原因：
   1. 防止已失效的连接请求又传送到服务器
      1. 非三次握手：建立新的链接，浪费资源
...
4. 四次挥手原因：
   ...
   2. 好处：
      1. 防止数据丢失
   3. 第二次握手的意义：
      1. 这是因为服务端在LISTEN状态下，收到建立连接请求的SYN报文后，把ACK和SYN放在一个报文里发送给客户端。而关闭连接时，当收到对方的FIN报文时，仅仅表示对方不再发送数据了但是还能接收数据，己方也未必全部数据都发送给对方了，所以己方可以立即close，也可以发送一些数据给对方后，再发送FIN报文给对方来表示同意现在关闭连接，因此，己方ACK和FIN一般都会分开发送。
   4. 第四次握手的TIME_WAIT意义：
      1. 为了保证客户端发送的最后一个ACK报文段能够到达服务器，即可靠的终止TCP连接。
         1. TIME_WAIT经过2MSL（最大报文段生存时间）到CLOSE。
         2. 如果网路不可靠，最后的ACK丢失。服务器不断重发FIN。客户端用TIME_WAIT重发ACK。
         3. 直到2MSL，客户端都未收到FIN，推断ACK成功接收，结束TCP链接。
      2. 为了防止失效的连接请求报文段出现在本连接中。
         1. 客户端在发送完最后一个ACK报文段过后，再经过2MSL就可以使本连接持续的时间内产生的所有报文段从网络中消失，这样就可以使下一个新的连接中不会出现这种旧的连接请求报文段

5. 其他问题：

   1. 如果已经建立了链接，但是客户端突发故障了怎么办：
   ...
   2. TIME_WAIT会带来哪些问题：
      1. 作为服务器，短时间内关闭了大量的客户端连接，会造成服务器上出现大量的TIME_WAIT连接，占据大量的tuple(源IP地址、目的IP地址、协议号、源端口、目的端口),严重消耗着服务器的资源
      2. 作为客户端，短时间内大量的短连接，会大量消耗C机器的端口，毕竟端口只有65535个，端口耗尽了，后续就无法启用新的连接了。

#### 13. TCP拥塞控制

1. [定义](https://blog.csdn.net/qq_41431406/article/details/97926927)：防止过多的数据注入网络中，这样可以使网络中的路由器或链路不致过载。
2. 与流量控制区别；
   1. 流量控制：点对点通信量的控制
   2. 拥塞控制：全局性过程
3. 拥塞控制方法：
   ...
   3. 快重传：
      1. 定义：
         1. 接收方不要等待自己发送数据时才捎带确认，**立即发送确认**
         2. 即使收到了失序的报文段也要立即发出对已收到报文段的重复确认。
         3. 发送方收到3个重复确认则立即重传未收到的报文段。不必等待设置的重传计时器到期。
      2. 好处：
         1. 防止TCP链接因为等待重传计时器太长。
         2. 对于个别丢失的报文段，发送方不会出现超时重传，也就不会误认为出现了阻塞（进而降低拥塞窗口为1），提高了网络吞吐量。
   4. 快恢复：发送方收到3个重复确认，就知道只是丢失了个别报文段。不启动慢开始算法，执行快恢复算法。
      1. 慢开始门限ssthresh和拥塞窗口调整为当前窗口的一半，开始执行拥塞避免算法。
      2. 也有一些快恢复实现把快恢复开始时的拥塞窗口cwnd再增大一些，即新的ssthresh+3。

#### 20. TCP流量控制

![img](img/%E9%9D%A2%E8%AF%95%E7%AD%94%E9%A2%98%E6%A1%86%E6%9E%B6/640.jpg)

![img](https://gitee.com/wychz/PicGoBed/raw/master/img/20210404182057.jpg)

1. 背景（滑动窗口机制）：
   1. 双方在通信的时候，发送方的速率与接收方的速率是不一定相等，如果发送方的发送速率太快，会导致接收方处理不过来，这时候接收方只能把处理不过来的数据存在缓存区里（失序的数据包也会被存放在缓存区里）
   2. 如果缓存区满了发送方还在疯狂着发送数据，接收方只能把收到的数据包丢掉，大量的丢包会极大着浪费网络资源，因此，我们需要控制发送方的发送速率，让接收方与发送方处于一种动态平衡才好。对发送方发送速率的控制，我们称之为流量控制。
2. 实现：
...
![img](https://images2015.cnblogs.com/blog/1039362/201705/1039362-20170510193442941-292450452.png)

tcp报文

![img](https://images2015.cnblogs.com/blog/1039362/201705/1039362-20170510193521910-1359088430.png)

udp报文

![img](https://images2015.cnblogs.com/blog/1039362/201705/1039362-20170510193530504-1021481554.png)

ip和udp都有16位的长度字段，所以ip和udp报文的最大负载长度应该为2^16,64KB

**网络层限制：**

　　MTU（Maximum Transmission Unit，最大传输单元），这限制了ip报文长度，ip报文长度大于mtu，就需要分片。通常设为1500字节。

　　在网络层，ip提供不可靠传输服务，一旦收到的数据报分片丢失部分或部分出现差错，就会全部丢弃，而不会选择重传。

**传输层限制：**

 　tcp协议中有个MSS，tcp通常将数据分成长度为MSS的若干块。
...
　　ip报文由于其长度字段限制，最大负载长度为64KB，而由mtu限制，报文会分片为长度为mtu的若干段。

　　 udp：因没有序号等字段，所以无法分片，最大负载长度64KB，受制ip报文限制，数据最大负载为64KB-20（ip）-8（udp）

　　 tcp：目前来看，tcp的最大负载限制应该是其分片序号，2^32*mss,如果按MSS=1460来算，接近6TB，但这绝不可能，毕竟还有重传等操作，不过我觉得可以认为tcp的最大负载很大。（—_—）
...
数据包是网络层传输的数据单元。也称为IP包，包中带有足够寻址信息(IP地址)，可独立地从源主机传输到目的主机。还有一种说法是数据报是网络层的传输基本单位，数据包是IP协议中完整的数据单元，由一个或多个数据报组成。（待确认）
...
1. 定义：完成IP地址和物理地址的映射
2. 工作原理：
   1. 每台主机在ARP缓冲区建立ARP列表。发送数据包时检查列表是否有对应的MAC地址。
   2. 有：发送到此。
...
1. 背景：HTTP是无状态协议，服务端需要用户的状态时，需要某种机制来识别用户。Cookie 和 Session是客户端和服务器之间保持状态的方案。
2. 使用：
   1. 购物车，你按下下单按钮时，服务端不知道是哪个用户，所以服务端为特定用户创建了session，标识并跟踪这个用户，才能知道购物车里有什么东西。
3. Cookie：一小段文本信息。浏览器用response发给客户端，客户端保存cookie。浏览器再次请求时把cookie发给服务器，让服务器检查用户状态。
   1. 使用Cookie实现Session跟踪，第一次创建Session的时候，服务端在HTTP协议中告诉客户端，在Cookie里保存一个SessionID。下次请求根据这个SessionID分辨用户。如果客户端禁用了Cookie，可以使用URL重写技术，每次HTTP交互在URL后面附上一个sid=xxxxx这样的参数。服务端由此分辨用户。
   2. COkkie还可用于方便用户，比如某次登录网站，账号信息可以写入到Cookie里面。下次访问网站的时候，网站脚本读取该信息，自动填用户名和密码，方便用户。
4. Session：服务端用session保存状态。
   1. 服务器有session，用sessionID检索出来用。
   2. 服务器无session，创建一个session然后关联sessionID，并将sessionID发给客户端cookie保存。
   3. 保存方式：保存在内存，数据库，文件。当构建集群时也要考虑session的转移，大型网站一般会有专门的Session服务器集群，用来保存用户会话，这个Session是保存在内存的，使用一些缓存服务比如Memcached去存放session。
   4. 用途：
      1. Session可以记录用户的登录与行为数据，即记录下用户目前访问服务器上的内容，状态。由于这部分内容用户修改的随机性很大，没有必要直接存储在数据库中
      2. 用户执行刷新操作时，可以根据session，打开用户上次访问时网页的状态（如用户输入的表单内容等），为用户带来更优的体验
      3. 用户的session记录了用户在进入页面，查看结果，点击结果以及后继操作（翻页，加购）。只有通过session信息才能联系起来用户的行为。
5. 对比：
...
6. Application：与一个WEB应用程序相对应，为应用程序提供一个全局状态。

### 14. 输入网址到获得页面

1. 浏览器查询DNS，获取域名对应的IP地址
   1. 先在浏览器缓存中查询，浏览器会缓存域名和ip地址的关系
   2. 去系统缓存中查询，浏览器发起一个系统调用，查询系统缓存中的数据
   3. 去路由器缓存中查询
   4. 从本地DNS服务器的缓存中查找，本地服务器即用户自己配置的DNS服务器
   5. 本地NDS发送请求至根域名服务器，根域名服务器没有数据再查com顶级域名服务器，再查baidu权威服务器，再查www服务器。这是一种递归的过程。即local至根域名，根域名不直接返回com地址，而是发送请求至com，com发送请求至baidu，baidu发送请求至www，www再返回给baidu，baidu返回给com，com再返回给local）至此，整个DNS查询步骤结束，现在浏览器拿到了域名对应的ip地址。
2. 三次握手建立链接浏览器发送HTTP请求
   1. 可能经历重定向过程：为什么服务器一定要重定向而不是直接发送用户想看的网页内容呢？其中一个原因跟搜索引擎排名有关。如果一个页面有两个地址，就像http://www.yy.com/和http://yy.com/，搜索引擎会认为它们是两个网站，结果造成每个搜索链接都减少从而降低排名。而搜索引擎知道301永久重定向是
   2. 什么意思，这样就会把访问带www的和不带www的地址归到同一个网站排名下。还有就是用不同的地址会造成缓存友好性变差，当一个页面有好几个名字时，它可能会在缓存里出现好几次。
3. 服务器根据路径参数映射到特定处理器，返回执行结果和http响应视图
   1. 对TCP连接进行处理，对HTTP协议进行解析，按照报文格式进一步封装成HTTP Request对象，供上层使用
   2. 返回响应
4. 流量器解析渲染视图。遇到静态资源重复上述请求。
   1. 浏览器解压缩响应体之后，在界面显示的同时，下载css，png等一些静态文件，这些静态文件可以被浏览器缓存，防止多次请求。很多公司也将静态文件保存在CDN（内容分发网络）上，从CDN上加载大量静态文件，加快查找速度。
      1. 解析HTML文件构建DOM树，然后间隙CSS文件构建渲染树，渲染构建完成之后，浏览器布局渲染树并绘制到屏幕上。
      2. 发送请求获取嵌入在HTML中的资源，如图片，音频，视频等。
5. 浏览器渲染页面。

### 15. DNS

1. 定义：DNS是一个域名和IP相互映射的分布式数据库，能够使用户更加方便的访问互联网，而不用记住IP串。通过主机名，得到该主机名对应的IP地址的过程叫做域名解析。
...
1.  攻击内容：
    ...
    synflood：客户端在短时间内伪造大量不存在的ip地址，向服务器不断发送syn包。由于源地址不存在，服务器不断重发直至超时，正常的SYN请求被丢弃，目标系统运行缓慢， 严重者导致网络阻塞。，
...
1. 协程不被操作系统内核所管理，完全由程序所控制（在用户态执行）
2. 协程运行在线程之上，当一个协程执行完成后，可以选择主动让出，让另一个协程运行在当前线程之上。**协程并没有增加线程数量，只是在线程的基础之上通过分时复用的方式运行多个协程**，而且协程的切换在用户态完成，切换的代价比线程从用户态到内核态的代价小很多。

### 2. 进程通信方式

1. 管道：半双工，单项流动，亲缘关系进程中流动

   1. 匿名管道：日常Linux系统中的`|`就是匿名管道。指令的前一个输入是后一个指令的输出。
   2. 命名管道：一般通过`mkfifo SoWhatPipe`创建管道。通过`echo "sw" > SoWhatPipe`跟`cat < SoWhatPipe` 实现输入跟输出。

   * shell管道通信：Shell执行匿名管道 a | b其实是通过Shell父进程fork出了两个子进程来实现通信的，而ab之间是不存在父子进程关系的。而命名管道是可以直接在不想关进程间通信的，因为有管道文件。

2. 信号：通知某个进程事件已经发生。对于异常状态下进程工作模式需要用到信号工作方式来通知进程。比如Linux系统为了响应各种事件提供了很多异常信号`kill -l`，**信号是进程间通信机制中唯一的异步通信机制**，可以在任何时候发送信号给某一进程。

   1. kill -9 1412 ，表示给 PID 为 1412 的进程发送 SIGKILL 信号，用来立即结束该进程。
   2. 键盘 Ctrl+C 产生 SIGINT 信号，表示终止该进程。
   3. 键盘 Ctrl+Z 产生 SIGTSTOP 信号，表示停止该进程，但还未结束。
...
3. 消息队列：消息队列是保存在**内核**中的消息链表，**会涉及到用户态跟内核态到来回切换**，双方约定好消息体到数据结构，然后发送数据时将数据分成一个个独立的数据单元消息体
   * 克服信号传递信息少，管道只能承载无格式字节流和缓冲区大小受限等缺点
   * 写权限的进程向队列添加消息，读权限的线程读取消息。

4. 共享内存：就是映射一段能被其他进程访问的内存。速度最快，常和信号量配合使用

5. 信号量：计数器，主要用于实现进程间的互斥与同步，控制多进程的对共享资源访问。控制信号量的方式有**PV两种原子操作**
...
6. 套接字： Socket 通信。进程间通信机制，可用于不同设备间。
...
1. 线性方式：
   1. 所有PCB组织在一张线性表中，将该表首地址存在内存的一个专用区域
   2. 实现简单，开销小但是每次都需要扫描整张表，适合进程数不多的系统
2. 索引方式：
...
1. 进程创建：父进程可创建子进程，父进程终止后子进程也会被终止。子进程可继承父进程所有资源，子进程终止需将自己所继承的资源归还父进程。
...
2. 进程终止：
   ...
   1. 正常结束
   2. 异常结束：越界错，保护错，运行超时，特权指令出错，非法指令
   3. 外界干预：操作员或OS干预（死锁），父进程请求，子进程完成父进程指定的任务时，父进程终止，所有子进程都应该结束
...
5. 多级队列调度算法
6. 多级反馈队列调度算法
   1. 系统会同时存在多个就绪队列，每个队列优先级从高到低排列，同时优先级越高获得是时间片越短。
   2. 新进程会先加入到最高优先级队列，如果新进程优先级高于当前在执行的进程，会停止当前进程转而去执行新进程。新进程如果在时间片内没执行完毕需下移到次优先级队列。

### 9. 僵尸进程

1. 定义：
   1. 僵尸进程是当子进程比父进程先结束，而父进程又没有回收子进程，释放子进程占用的资源，此时子进程将成为一个僵尸进程。
   2. 一个进程在调用exit命令结束自己的生命的时候，其实它并没有真正的被销毁，而是留下一个称为[僵尸进程](http://www.nowamagic.net/librarys/veda/tag/僵尸进程)（Zombie）的数据结构（系统调用exit， 它的作用是使进程退出，但也仅仅限于将一个正常的进程变成一个僵尸进程，并不能将其完全销毁）。
   3. 在Linux进程的状态中，僵尸进程是非常特殊的一种，它已经放弃了几乎所有内存空间，没有任何可执行代码，也不能被调度，仅仅在进程列表中保留一个位 置，记载该进程的退出状态等信息供其他进程收集。除此之外，僵尸进程不再占有任何内存空间。它需要它的父进程来为它收尸，如果他的父进程没安装 SIGCHLD 信号处理函数调用wait或waitpid()等待子进程结束，又没有显式忽略该信号，那么它就一直保持僵尸状态，如果这时父进程结束了， 那么init进程自动会接手这个子进程，为它收尸，它还是能被清除的。但是如果如果父进程是一个循环，不会结束，那么子进程就会一直保持僵尸状态，这就是 为什么系统中有时会有很多的僵尸进程。
2. 查看：
   1. 利用命令ps，可以看到有父进程ID为1的进程是孤儿[进程](http://www.nowamagic.net/librarys/veda/tag/进程)；s(state)状态为Z的是僵尸进程。注意：孤儿进程(orphan process)是尚未终止但已停止(相当于前台挂起)的进程，但其父进程已经终止，由init收养；而僵尸进程则是已终止的进程，其父进程不一定终止
3. 清除：
...
在Linux系统中一切皆文件，Linux文件系统会为每个文件分配`索引节点 inode`跟`目录项directory entry`来记录文件内容跟目录层次结构。

1. inode：

   1. 文件数据都储存在块中，我们还必须找到一个地方储存文件的元信息，比如inode编号、文件大小、创建时间、修改时间、磁盘位置、访问权限等。几乎除了文件名以为的所有文件元数据信息都存储在一个叫叫索引节点inode的地方。可通过`stat 文件名`查看 inode 信息
   2. Unix/Linux系统内部不使用文件名，而使用inode号码来识别文件，对于系统来说文件名只是inode号码便于识别的别称或者绰号
      1. 打开文件步骤：
...
2. 目录：
   ...
   1. **目录directory也是一种文件**。目录文件内容就是一系列目录项的列，目录项的内容包含**文件的名字、文件类型、索引节点指针以及与其他目录项的层级关系**。


### 1. 硬链接 软链接

软链接：相当于基于老文件A新建了个文件B，该文件B有新的inode，不过文件B内容是老文件A的路径。当老文件A删除后，文件B仍然存在，不过找不到指定文件了。
...
**文件系统操作基本单位是数据块**。 文件在磁盘中存储时候主要分为`连续空间存储`跟`非连续空间存储`
...
[详细](http://c.biancheng.net/view/3066.html)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/wJvXicD0z2dWtyQ6fLJR8SwofF59o2Iq2Cic1HIGee0USUmtB8vhLaIgcnr9s1zbSEqiaibCc2rQS7rGiaI2YOpUINQ/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

### 3.IO模式

1. 轮询模式：控制器中有个**状态寄存器**，CPU不断**轮**询查看寄存器状态，该模式会傻瓜式的一直占用CPU。

2. 中断模式：控制器有个中断控制器，当设备完成任务后触发中断到中断控制器，中断控制器就通知 CPU来处理中断请求。中断有两种，一种是**软中断**，比如代码调用 INT 指令触发。一种是**硬件中断**，硬件通过中断控制器触发的。但中断方式对于频繁读写磁盘数据的操作就不太友好了，会频繁打断CPU。

   这里说下磁盘高速缓存 **PageCache**，它是用来缓存最近被CPU访问的数据到内存中，并且还具有预读功能，可能你读前16KB数据，已经把后16KB数据给你缓存好了。

3. DMA模式：`Direct Memory Access` 直接内存访问，在硬件DMA控制器的支持下，**在进行 I/O 设备和内存的数据传输的时候，数据搬运的工作全部交给 DMA 控制器，而 CPU 不再参与任何与数据搬运相关的事情，让CPU 去处理别的事**。

4. 零拷贝模式：Zero Copy 全程不会通过 CPU 来搬运数据，所有的数据都是通过 DMA 来进行传输的，中间只需要经过2次上下文切换跟2次DMA数据拷贝，相比最原始读写方式至少速度翻倍。

   1. 老版本：期间会发生**4次用户态跟内核态的切换。2次DMA数据拷贝，2次CPU数据拷贝**。

      ![图片](https://mmbiz.qpic.cn/mmbiz_png/wJvXicD0z2dWtyQ6fLJR8SwofF59o2Iq2ic61AurriamwAQMneiawhQ7w9bkIuVfyVt5XhCFKun8SqsQ3aBHQBzy7w/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

   2. mmap：mmap调用时会**直接把内核缓冲区里的数据映射到用户空间**，此时减少了一次数据拷贝，但仍然需要通过 CPU 把内核缓冲区的数据拷贝到 socket 缓冲区里，而且仍然需要 4 次上下文切换

      ![图片](https://mmbiz.qpic.cn/mmbiz_png/wJvXicD0z2dWtyQ6fLJR8SwofF59o2Iq2johrpubN4b8EMyKG4sYyIAjibBsKJTzYmmMJIGPd3hmor9xdCefpibAw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

   3. sendfile：一个 sendfile = read + write，避免了2次用户态跟内核态来回切换，并且可以直接把内核缓冲区里的数据拷贝到 socket 缓冲区里，这样就只有 2 次上下文切换，和 3 次数据拷贝。

      ![图片](https://mmbiz.qpic.cn/mmbiz_png/wJvXicD0z2dWtyQ6fLJR8SwofF59o2Iq2bicp2pAamEuiaUBCicokYCSSvh1b3kQeBibAbjp1CjhPkoqqJfWwn7vGUw/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)

   4. 网卡SG-DMA技术：SG-DMA 技术可以直接将内核缓存中的数据拷贝到网卡的缓冲区里，此过程不需要将数据从操作系统内核缓冲区拷贝到 socket 缓冲区中，这样就减少了一次数据拷贝

      ![图片](https://mmbiz.qpic.cn/mmbiz_png/wJvXicD0z2dWtyQ6fLJR8SwofF59o2Iq2uDxicTRj2iaoO1emHgugDsibIkR8NqUWIubrRcibL9503NKKht9gm6OP3Q/640?wx_fmt=png&tp=webp&wxfrom=5&wx_lazy=1&wx_co=1)
...
![8ec87d116ce0badc66f33501adc6b490.png](https://img-blog.csdnimg.cn/img_convert/8ec87d116ce0badc66f33501adc6b490.png)

InnoDB的逻辑存储结构：node

表空间：

能够看作是InnoDB存储引擎逻辑结构的最高层，全部的数据都存放在表空间中。默认有个共享表空间ibdata1。若是启用innodb_file_per_table参数，须要注意每张表的表空间内存放的只是数据、索引和插入缓冲Bitmap页，其余类的数据，如回滚信息、插入缓冲索引页、系统事务信息、二次写缓冲等仍是存放在原来共享表空间中。数据库
...
3. [为什么采用B+树](https://www.cnblogs.com/kungfupanda/p/12776674.html)
   
   1. hash：虽然可以快速定位，但是没有顺序，IO复杂度高。二叉树：树的高度不均匀，不能自平衡，查找效率跟数据有关（树的高度），并且IO代价高。红黑树：树的高度随着数据量增加而增加，IO代价高。
   2. B+树的磁盘读写代价更低，查询效率更加稳定。B+树只要遍历叶子结点就可以实现整棵树的遍历。而且在数据库中基于范围的查询非常频繁，而B树只能中序遍历所有节点，效率低。
...
4. 为什么要采用自增长主键：
   ...
   1. 自增主键是连续的，在插入过程中尽量减少页分裂，即使要进行页分裂，也只会分裂很少一部分。并且能减少数据的移动，每次插入都是插入到最后。总之就是减少分裂和移动的频率。
   
5. B+树和B树的区别

   1. B树：

      ![img](https://upload-images.jianshu.io/upload_images/1446087-bc023e47bc74cfa1.jpg?imageMogr2/auto-orient/strip|imageView2/2/w/800/format/webp)
   ...
   2. B+树：
      ...
      1. **内节点并不存储 data，所以一般B+树的叶节点和内节点大小不同，而B-树的每个节点大小一般是相同的，为一页。** 所有关键字存储在叶子结点，内部节点（非叶子结点）并不存储真正的data
      2. 为所有叶子结点增加了一个链指针

6. 为什么不用红黑树或者二叉排序树？

   1. 树的查询时间跟树的高度有关，B+树是一棵多路搜索树可以降低树的高度，提高查找效率

7. 既然增加树的路数可以降低树的高度，那么无限增加树的路数是不是可以有最优的查找效率？

   1. 这样会形成一个有序数组，文件系统和数据库的索引都是存在硬盘上的，并且如果数据量大的话，不一定能一次性加载到内存中。有序数组没法一次性加载进内存，这时候B+树的多路存储威力就出来了，可以每次加载B+树的一个结点，然后一步步往下找，
...
3. 分类方式3：
   ...
   1. [聚簇索引](https://blog.csdn.net/weixin_33134753/article/details/113442616)（聚集索引）
      1. 定义：
         1. 聚簇索引就是按照每张表的主键构造一颗B+树，同时叶子节点中存放的就是整张表的行记录数据，也将聚集索引的叶子节点称为数据页。这个特性决定了索引组织表中数据也是索引的一部分，每张表只能拥有一个聚簇索引。这个索引的key是数据表的主键，因此InnoDB表数据文件本身就是主索引，按B+Tree组织的一个索引结构，这棵树的叶节点data域保存了完整的数据记录。
         2. Innodb通过主键聚集数据
   ...
   2. 非聚集索引（非聚簇索引）
      1. 定义：
         1. 在**聚簇索引之上创建的索引称之为辅助索引**，辅助索引访问数据总是需要二次查找。辅助索引叶子节点存储的不再是行的物理位置，而是主键值。通过辅助索引首先找到的是主键值，再根据这个地址读取对应的数据记录。（再通过主键值找到数据行的数据页，再通过数据页中的Page Directory找到数据行。）
         2. Innodb辅助索引的叶子节点并**不包含行记录的全部数据**，叶子节点除了包含键值外，还包含了相应行数据的聚簇索引键。
         3. 辅助索引的存在不影响数据在聚簇索引中的组织，所以一张表可以有多个辅助索引。在innodb中有时也称辅助索引为二级索引。索引和辅助索引（Secondary key）在结构上没有任何区别，**只是主索引要求key是唯一的，而辅助索引的key可以重复**。
...
4. 行数：MyISAM存储表的总行数，InnoDB不存储总行数
5. 索引：MyISAM采用非聚集索引，B+树叶子存储指向数据文件的指针。InnoDB采用聚集索引，B+树叶子存储数据。
6. 使用场景：
   1. MyISAM：插入不频繁，查询频繁
...
1. id：select查询中的序号
   ...
   1. 先执行序号大的，先从下而上执行。遇到序号相同时，再从上而下执行。
   2. id列是可以允许为空的，并且是在SELECT类型为： UNION RESULT的时候

2. select_type：该列表示select的类型
...
3. table：该列的值表示输出行所引用的表的名称

4. partitions：该列的值表示查询将从中匹配记录的分区

5. type：该列的值表示连接类型，是查看索引执行情况的一个重要指标
...
7. key：实际用到的索引
8. key_len：表示使用索引的长度。上面的key列可以看出有没有使用索引，key_len列则可以更进一步看出索引使用是否充分。它是最重要的列。
9. ref：索引命中的列或者常量
10. rows：表示MySQL认为执行查询必须检查的行数
11. filtered：按表条件过滤的表行的估计百分比
12. Extra：包含有关MySQL如何解析查询的其他信息
    1. **Impossible WHERE**：表示WHERE后面的条件一直都是false
    2. **Using filesort**：表示按文件排序，一般是在指定的排序和索引排序不一致的情况才会出现
    3. **Using index**：表示是否用了覆盖索引，说白了它表示是否所有获取的列都走了索引。
    4. **Using temporary**：表示是否使用了临时表，一般多见于order by 和 group by语句
    5. **Using where**：表示使用了where条件过滤。
    6. **Using join buffer**：表示是否使用连接缓冲。来自较早联接的表被部分读取到联接缓冲区中，然后从缓冲区中使用它们的行来与当前表执行联接。
...
6. 尽量使用覆盖索引。如果一个索引包含所有需要查询的字段的值，直接返回数据而不需读表。极大提高性能。因此可以让索引包含额外的列，即使这个列对索引是无用的
...
1. [定义](https://www.cnblogs.com/sivkun/p/7518540.html)：多个事物在统一资源上相互占用并请求锁定对方占用的资源导致恶性循环。当多个事物试图以不同顺序锁定资源就会产生死锁，多个事务同时锁定同一个资源也会产生死锁。
2. 解决方法：
...
2. 选择合适的表字段数据类型和存储引擎，为搜索字段创建索引，适当的添加索引。
3. mysql库主从读写分离，分库分表
4. 找规律分表，减少单表中的数据量提高查询速度。
5. 添加缓存机制，比如memcached，apc等。
6. 不经常改动的页面，生成静态页面。
7. 避免使用select*，列出需要查询的字段，书写高效率的SQL。比如 SELECT * FROM TABEL 改为 SELECT field_1, field_2, field_3 FROM TABLE.
8. 系统配置的优化
9. 硬件优化
...
1. Redo Log：
   1. 背景：当数据库对数据做修改的时候，需要把数据页从磁盘读到buffer pool中，然后在buffer pool中进行修改，那么这个时候buffer pool中的数据页就与磁盘上的**数据页内容不一致**，称buffer pool的数据页为dirty page 脏数据，如果这个时候发生非正常的DB服务重启，那么这些数据还没在内存，并**没有同步到磁盘文件中**（注意，同步到磁盘文件是个随机IO），也就是会发生数据丢失，如果这个时候，能够在有一个文件，当buffer pool 中的data page变更结束后，**把相应修改记录记录到这个文件**（注意，记录日志是顺序IO），那么当DB服务发生crash的情况，**恢复DB的时候，也可以根据这个文件的记录内容，重新应用到磁盘文件，数据保持一致**。 这个文件就是redo log ，用于记录 数据修改后的记录，顺序记录。
   2. 好处：
...
1. 原理：AOF持久化以独立日志的方式记录每次写命令，重启时再重新执行AOF文件中的命令达到恢复数据的目的。AOF的作用主要解决了数据持久化的实时性。
2. 工作流程：
...
3. 文件同步策略：
   1. Redis提供多种AOF缓冲区文件同步策略，由参数`appendfsync`控制，不同的值含义如下：
...
2. skiplist跳跃表：跳表是一种**单层多指针的链表**。查找效率很高，堪比优化过的二叉平衡树。
   ...
   跳表就是多层**链表**的结合体，跳表分为许多层(level)，每一层都可以看作是数据的**索引**，这些索引的意义就是加快跳表查找数据速度。每一层的数据都是有序的，上一层数据是下一层数据的子集，并且第一层(level 1)包含了全部的数据；层次越高，跳跃性越大，包含的数据越少。并且随便插入一个数据该数据是否会是跳表索引完全随机的跟玩骰子一样，redis中的`zset`底层就是跳表数据结构。并且跳表的速度**几乎接近**红黑树了。跳表包含一个表头，它查找数据时，是`从上往下，从左往右`进行查找。
   ...
   * 跳表实现简单速度挺好`ConcurrentHashMap`为什么不直接用跳表用红黑树？先说下在`HashMap`中一般空间利用率就在40%作用，而`ConcurrentHashMap`空间利用率只能达到10%～20%。如果这个时候再不节省空间还用跳表替换红黑树。那么就凉凉了。
...
2. 实现：使用SETNX命令实现分布式锁。将 key 的值设为 value ，当且仅当 key 不存在。命令在设置成功时返回 `1` ， 设置失败时返回 `0` 。
3. 解锁：del key。
4. 解决死锁：
...
4. Broker：一台kafka服务器就是一个broker。一个集群由多个broker组成。每个broker容纳多个topic。broker是消息的代理，Producers往brokers里面的指定topic写消息。Consumers从broker里面拉取指定topic 的消息，然后进行业务处理。broker起到一个代理保存消息的中转站。
5. Topic：可以理解为一个队列，生产者和消费者面向的都是一个topic。
   1. Kafka消息以主题为单位进行归类，生产者发送消息至主题，消费者订阅主题进行消费
   2. 主题是逻辑概念。一个主题 = 多个分区。不同分区包含消息不同。
6. Partition：一个topic分布到多个broker（服务器）上，一个topic可以分为多个partition。每个partition是一个有序队列。partition是物理概念。
   1. 分区可看做一个可追加日志，消息被追加到分区日志文件的时候回分配一个特定的偏移量offset。每个partition对应一个log文件，存储producer生产的数据。producer生产的数据会不断追加到log文件末端，每条数据都有自己的offset。消费者实时记录自己消费到了哪个offset，出错恢复时从上次的位置继续消费。
...
3. offset提交：
   ...
   2. 在旧消费者客户端中，消费位移是存储在 ZooKeeper 中的。而在新消费者客户端中，消费位移存储在 Kafka 内部的主题__consumer_offsets 中。

   3. 当前消费者需要提交的消费位移并不是x，而是**x+1**，它表示下一条需要拉取的消息的位置。
...
1. 概念：
   ...
   3. leader维护一个与其基本同步的Replica列表，成为ISR（in-sync Replica），每个Partition都有一个ISR，由leader动态维护。，follower从leader获取同步数据有一些延迟（延迟时间，延迟条数维度）。任意一个超过阙值会将follower踢出ISR，存入OSR（Outof-Sync Replicas）列表。新加入的follower会先存放在OSR中，AR=ISR+OSR。
2. 使用：
...
2. kafka集群中有一个broker会被选举为Controller，负责管理集群broker的上下线，所有topic的分区副本分配和leader选举等工作。Controller的选举依赖于Zookeeper。

### 8. kafka效率

1. Cache FileSystem Cache PageCache缓存。

   1.  Kafak大量使用页缓存，消息先被写入页缓存，操作系统负责具体的刷盘操作，在合适的时间将脏页写入磁盘。
...
[详细](https://www.cnblogs.com/smartloli/p/11922639.html)
...
[详细1](https://zhuanlan.zhihu.com/p/112745985)

* Kafka的位移提交方式为定期自动提交，默认每隔5秒进行一次。在向服务端发起拉取请求之前检查是否可以进行位移提交，可以则提交上一次轮询的位移。这种方式会造成**重复消费**。也就是在**经过auto.commit.interval.ms间隔后，并且在下一次调用poll时才会提交所有 已消费消息的offset。**可以通过减小时间间隔的方式解决。
  * 假设刚刚提交完一次消费位移，然后拉取一批消息进行消费，在下一次自动提交消费位移之前，消费者崩溃了，那么又得从上一次位移提交的地方重新开始消费
  * **消费者消费时间过长**.max.poll.interval.ms参数定义了两次poll的最大间隔，它的默认值是 5 分钟，表示你的 Consumer 程序如果在 5 分钟之内无法消费完 poll 方法返回的消息，那么 Consumer 会主动发起“离开组”的请求，Coordinator 也会开启新一轮 Rebalance。若消费者消费的消息比较耗时，那么这种情况可能就会出现。在消费过程中消费者单次会拉取11条消息，每条消息耗时30s，11条消息耗时 5分钟30秒，由于max.poll.interval.ms 默认值5分钟，所以理论上消费者无法在5分钟内消费完，consumer会离开组，导致rebalance。可以看到在消费完第11条消息后，因为消费时间超出max.poll.interval.ms 默认值5分钟，这时consumer已经离开消费组了，开始rebalance，因此提交offset失败。之后重新rebalance，消费者再次分配partition后，再次poll拉取消息依然从之前消费过的消息处开始消费，这样就造成重复消费。而且若不解决消费单次消费时间过长的问题，这部分消息可能会一直重复消费。
...
3. 解决消息重复消费
   ...
   **解决重复消费问题**：设置一个递增的序号来维护异步提交的顺序，每次位移提交之后就增加序号相对应的值。在遇到位移提交失败需要重试的时候，可以检查所提交的位移和序号的值的大小，如果前者小于后者，则说明有更大的位移已经提交了，不需要再进行本次重试；如果两者相同，则说明可以进行重试提交。
   ...
   1. 提高消费能力，提高单条消息的处理速度，例如对消息处理中比 较耗时的步骤可通过异步的方式进行处理、利用多线程处理等。在缩短单条消息消费时常的同时，根据实际场景可将max.poll.interval.ms值设置大一点，避免不 必要的rebalance，此外可适当减小max.poll.records的值，默认值是500，可根 据实际消息速率适当调小。这种思路可解决因消费时间过长导致的重复消费问题， 对代码改动较小，但无法绝对避免重复消费问题。
   2. 引入单独去重机制，例如生成消息时，在消息中加入唯一标识符如消息id等。在消费端，我们可以保存最近的1000条消息id到redis或mysql表中，配置max.poll.records的值小于1000。在消费消息时先通过前置表去重后再进行消息的处理。
   
4. 解决消息丢失：手动提交：分为同步提交和异步提交。

   从commitAsync()方法入手，根据源码进行分析
...
Kafka消息追加到指定分区中，并分配唯一的序列号（偏移量）。为防止log文件过大，kafka采用分片+索引机制。

1. Log在物理上以文件夹形式存储。一个日志Log被平均分配为多个相对较小的文件LogSegment。每个LogSegment=一个日志+两个索引。每个LogSegment都有偏移量索引文件和时间戳索引文件。
2. index和log文件以当前segment的第一条消息的offset命名。
3. index文件中的元数据指向log文件中message的物理偏移地址。
4. 向Log的追加消息，只有最后一个LogSegment（ActiveSegment）才能执行写入操作。
...
![image-20210329232149086](https://gitee.com/wychz/PicGoBed/raw/master/img/20210714233544.png)

![image-20210513195112952](img/%E9%9D%A2%E8%AF%95%E7%AD%94%E9%A2%98%E6%A1%86%E6%9E%B6/image-20210513195112952.png)

### 0. Netty流程

1. Netty抽象出两组线程池：BossGroup负责接收客户端的连接，WorkerGroup负责网络的读写
   1. BossGroup和WorkerGroup都是NioEventLoopGroup
   2. NioEventLoopGroup相当于一个事件循环组，组中含有多个事件循环，即NioEventLoop
   3. NioEventLoop表示一个不断循环的执行处理任务的下城，每个EventLoop都有一个selector，用于监听绑定在其上的socket的网络通讯

2. BossEventLoop执行流程：
   1. 轮询accept事件
   2. 处理accept事件，与client建立连接，生成NioSocketChannel，并将其注册到某个WorkerGroup的NioEventLoop的selector上
   3. 处理任务队列的任务，即runAllTasks
3. WorkerEventLoop执行流程：
   1. 轮询read，write事件
   2. 处理io事件，在对应的NioSocketChannel里面处理
   3. 处理任务队列的任务，即runAllTasks
   4. 处理业务时，会使用pipeline，通过handler处理。
...
1. 服务端
   ...
   3. 通过 .group() 方法给引导类 ServerBootstrap 配置两大线程组，确定了线程模型。ServerBootstrap需要配置两个线程组— EventLoopGroup ，一个用于接收连接，一个用于具体的处理。
   ...
   6. 调用 ServerBootstrap 类的 bind()方法绑定端口。Bootstrap 通常使用 connet() 方法连接。另外，Bootstrap 也可以通过 bind() 方法绑定本地的一个端口，作为 UDP 协议通信中的一端。

2. 客户端
   1. 创建一个 NioEventLoopGroup 对象实例
   2. 创建客户端启动的引导类是 Bootstrap
   3. 通过 .group() 方法给引导类 Bootstrap 配置一个线程组。Bootstrap 只需要配置一个线程组— EventLoopGroup
   ...
   6. 调用 Bootstrap 类的 connect()方法进行连接到远程的主机和端口，作为一个 Netty TCP 协议通信中的客户端，这个方法需要指定两个参数：host，port。

### 9. TCP沾包/拆包

[TCP 粘包/拆包](https://blog.csdn.net/john1337/article/details/102806307) 就是你基于 TCP 发送数据的时候，出现了多个字符串“粘”在了一起或者一个字符串被“拆”开的问题。
...
3. 数据结构：
![、](https://gitee.com/wychz/PicGoBed/raw/master/img/20210317204810)
...
* 简介：`Watcher` 为事件监听器，是 `zk` 非常重要的一个特性，很多功能都依赖于它，它有点类似于订阅的方式，即客户端向服务端 **注册** 指定的 `watcher` ，当服务端符合了 `watcher` 的某些事件或要求则会 **向客户端发送事件通知** ，客户端收到通知后找到自己定义的 `Watcher` 然后 **执行相应的回调方法** 。
...
2. 崩溃恢复：
   ...
   * Leader服务器出现宕机，或者因为网络原因导致Leader服务器失去了与过半 Follower的联系，那么就会进入崩溃恢复模式从而选举新的Leader。Leader选举算法不仅仅需要让Leader自己知道其自身已经被选举为Leader，同时还需要让集群中的所有其他服务器也能够快速地感知到选举产生的新的Leader。当选举产生了新的Leader，同时集群中有过半的服务器与该Leader完成了状态同步之后，Zab协议就会退出崩溃恢复模式，进入消息广播模式

3. 消息广播模式：**在整个集群中保持数据的一致性**。只有 `Leader` 能处理写请求，让`Follower` 和 `Observer` 也 **同步更新数据**。

   * Zab协议的消息广播过程类似二阶段提供过程，是一种原子广播的协议。当接受到来自Client的事务请求(如服务注册)(所有的事务请求都会转发给Leader)，Leader会为事务生成对应的Proposal，并为其分配一个全局唯一的ZXID。Leader服务器与每个Follower之间都有一个单独的队列进行收发消息，Leader将生成的Proposal发送到队列中。Follower从队列中取出Proposal进行事务消费，消费完毕后发送一个ACK给Leader。当Leader接受到半数以上的Follower发送的ACK投票，它将发送Commit给所有Follower通知其对事务进行提交，Leader本身也会提交事务，并返回给处理成功给对应的客户端。Follower只有将队列中Proposal都同步消费后才可用。
...
1. 定义：分布式系统是由一组通过网络进行通信、为了完成共同的任务而协调工作的计算机节点组成的系统。分布式系统的出现是为了用廉价的、普通的机器完成单个计算机无法完成的计算、存储任务。其目的是**利用更多的机器，处理更  多的数据**。
...
当收到来自集群中过半数节点的接受投票后，节点即成为Leader，开始接收Client的事务处理和查询并向其它的Follower节点同步事务。Leader节点会定时向Follower发送心跳来保持其地位。
...
1. Consistency：指数据的强一致性。如果写入某个数据成功，之后读取，读到的都是新写入的数据；如果写入失败，读到的都不是写入失败的数据。（等同于所有节点访问同一份最新的数据副本）相当于要求分布式系统中的各节点时时刻刻保持数据的一致性。
2. Availability：指服务的可用性。 一直可以正常的做读写操作。简单而言就是客户端一直可以正常访问并得到系统的正常响应。用户角度来看就是不会出现系统操作失败或者访问超时等问题。
3. Partition-tolerance：指分区容错。指的分布式系统中的某个节点或者网络分区出现了故障的时候，整个系统仍然能对外提供满足一致性和可用性的服务。也就是说部分故障不影响整体使用。
...
2. [幂等性实现方式](https://www.cnblogs.com/jajian/p/10926681.html)：
   ...
   2. 数据库去重表：
      ...
      1. 往去重表里插入数据的时候，利用数据库的唯一索引特性，保证唯一的逻辑。唯一序列号可以是一个字段，例如订单的订单号，也可以是多字段的唯一性组合。
      2. 比如每个请求有一个唯一标志。如订单ID。每次处理完请求后，有一个记录标识这个请求处理过了。比如在Mysql中记录个状态。支付订单必须要插入一条支付流水记录。order_id唯一键。支付订单前插入一条支付流水，order_id就进去了。如果有重复支付的时候，会再次插入订单流水。会报唯一键冲突。
...
2. 基于Redis：
   ...
   1. 实现：
      ...
      2. [RedLock算法](https://www.cnblogs.com/rgcLOVEyaya/p/RGC_LOVE_YAYA_1003days.html)：
...
[详细1](https://zhuanlan.zhihu.com/p/183753774)

[详细2](https://www.cnblogs.com/mayundalao/p/11798502.html)
...
漏桶：漏桶一个固定容量的漏桶，按照固定常量速率流出请求，流入请求速率任意，当流入的请求数累积到漏桶容量时，则新流入的请求被拒绝。漏桶可以看做是一个具有固定容量、固定流出速率的队列，漏桶限制的是请求的流出速率。漏桶中装的是请求。

令牌桶：令牌桶算法的原理是系统会以一个恒定的速度往桶里放入令牌，而如果请求需要被处理，则需要先从桶里获取一个令牌，当桶里没有令牌可取时，则拒绝服务

计数器：有时我们还会使用计数器来进行限流，主要用来限制一定时间内的总并发数，比如数据库连接池、线程池、秒杀的并发数；计数器限流只要一定时间内的总请求数超过设定的阀值则进行限流，是一种简单粗暴的总数量限流，而不是平均速率限流。
...
[详细1](https://stor.51cto.com/art/201909/602413.htm)   

**1.先更新缓存,再写数据库**（太差，不考虑）
...
1. 首先写入数据库。
2. 发送带有缓存key和value的事务消息到消息队列中。此时，需要有支持事务消息特性的消息队列。
3. 消费者消费该消息，更新到缓存中。更新失败则重试
   1. 最终解决：
...
![](../../%25E7%259F%25A5%25E8%25AF%2586%25E6%2580%25BB%25E7%25BB%2593/%25E9%25A1%25B9%25E7%259B%25AE/img/%25E9%25A1%25B9%25E7%259B%25AE/1.png)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20190322171441544.png)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20190322171742842.png)

![在这里插入图片描述](https://img-blog.csdnimg.cn/20190322171749481.png)
...
当服务提供者由于各种原因致使提供的服务停止时，需要向注册中心
...
4. Bean的生命周期
   ...
   3. **处理Aware接口**：Spring监测该对象是否实现了xxxAware接口，通过接口拿到Spring容器的资源。
      ...
      2. 如果bean实现了[BeanFactoryAware](https://blog.csdn.net/qcl108/article/details/101519602?dist_request_id=1328769.61172.16176322078188389&depth_1-)接口，工厂通过传递自身的实例来调用[setBeanFactory](http://www.pinhuba.com/spring/101249.htm)()
...
[详细](https://blog.csdn.net/baidu_19473529/article/details/81152109)
...
[详细](https://zhuanlan.zhihu.com/p/101128672)

ApplicationEvent，

ApplicationListener，监听某个Event，重写`onApplicationEvent(OrderSuccessEvent event)`方法。

### 20. FactoryBean

[详细](https://blog.csdn.net/wangbiao007/article/details/53183764?dist_request_id=&depth_1-)
...
CPU 异常往往还是比较好定位的。原因包括业务逻辑问题(死循环)、频繁 GC 以及上下文切换过多。

1. 业务逻辑问题：

   1. 先用 ps 命令找到对应进程的 pid(如果你有好几个目标进程，可以先用 top 看一下哪个占用比较高)。

   2. 接着用top -H -p pid来找到单个进程内 CPU 使用率比较高的一些线程，然后将占用最高的 pid 转换为 16 进制 printf '%x\n' pid 得到 nid：

   3. 接着直接在 jstack 中找到相应的堆栈信息 jstack pid |grep 'nid' -C5 –color：

   4. 仔细分析。当然更常见的是我们对整个 jstack 文件进行分析，通常我们会比较关注 WAITING 和 TIMED_WAITING 的部分，BLOCKED 就不用说了。

      我们可以使用命令 cat jstack.log | grep "java.lang.Thread.State" | sort -nr | uniq -c 来对 jstack 的状态有一个整体的把握，如果 WAITING 之类的特别多，那么多半是有问题啦。

2. 频繁GC：
   1.  jstat -gc pid 1000 命令来对 GC 分代变化情况进行观察，1000 表示采样间隔(ms)，S0C/S1C、S0U/S1U、EC/EU、OC/OU、MC/MU 分别代表两个 Survivor 区、Eden 区、老年代、元数据区的容量和使用量。
   2. 如果看到 GC 比较频繁，再针对 GC 方面做进一步分析，具体可以参考一下 GC章节的描述。
3. 上下文切换：
   1. 使用 vmstat 命令来进行查看：
   2. cs(context switch)一列则代表了上下文切换的次数。如果我们希望对特定的 pid 进行监控那么可以使用 pidstat -w pid 命令，cswch 和 nvcswch 表示自愿及非自愿切换。

### 2. 磁盘排查

1. 首先是磁盘空间方面，我们直接使用 df -hl 来查看文件系统状态：磁盘问题还是性能上的问题。我们可以通过 iostat -d -k -x 来进行分析。最后一列 %util 可以看到每块磁盘写入的程度，而 rrqpm/s 以及 wrqm/s 分别表示读写速度，一般就能帮助定位到具体哪块磁盘出现问题了。
2. 另外我们还需要知道是哪个进程在进行读写，一般来说开发自己心里有数，或者用 iotop 命令来进行定位文件读写的来源。不过这边拿到的是 tid，我们要转换成 pid，可以通过 readlink 来找到 pidreadlink -f /proc/*/task/tid/../..。找到 pid 之后就可以看这个进程具体的读写情况 cat /proc/pid/io：
3. 我们还可以通过 lsof 命令来确定具体的文件读写情况 lsof -p pid：

### 1. OOM排查

1. dump内存。使用jmap命令dump内存，需要注意的是，在linux JDK1.6某个版本里使用jmap可能会让系统挂掉，可以通过-d64来解决。

   1. `jmap -J-d64 -dump:format=b,file=dump.bin PID`

   2. -XX：+HeapDumpOnOutOfMemoryError（开启堆快照）
...
2. 查看gc日志。如果没有任何JVM参数设置，gc日志默认打印在stdout.log文件里，里面可能会打其他的日志，而且GC日志也不会输出时间，所以在JVM启动参数里最好加以下命令，规范下GC日志输出到/home/admin/logs/gc.log，并且打印GC时间。

   1. `-XX:HeapDumpPath=/home/admin/logs -Xloggc:/home/admin/logs/gc.log -XX:+PrintGCDetails -XX:+PrintGCDateStamps`
   2. 在查看GC日志里发现concurrent mode failure问题。这个问题是因为CMS（Concurrent Mark-Sweep）垃圾回收器在做fullgc，还没到达回收的阶段，但是年轻代又有新对象晋升到年老代，而年老代又没有足够空间了，只能全停机，进行fullgc。但是从日志上看没有释放多少空间，要么是堆不够大，要么是泄露，先调大堆，如果是泄露，还是会重现，如果不是泄露，就稳定了。于是我们进行了JVM参数调整,系统有8G内存，我们把JVM堆内存升到3.8G，修改参数XX:CMSInitiatingOccupancyFraction=60，让年老代在达到60%的时候进行CMS回收，这样在进行回收时候年老贷至少还有3800*0.4=1520m空间，就算把整个年轻代塞进去也没有问题。参数调整后，过了几天还是出现OOM，可以断定是内存溢出导致的，通过自动dump下来的内存文件很快发现有一个对象占用内存非常大，解决后系统恢复正常。

3. 使用Memory Analyzer (MAT)来分析
...
GC问题主要通过查看GC日志实现。在启动参数中加上 -verbose:gc，-XX:+PrintGCDetails，-XX:+PrintGCDateStamps，-XX:+PrintGCTimeStamps 来开启 GC 日志。

1. youngGC频繁：

   1. 先考虑是不是 Eden 区/新生代设置的太小了，看能否通过调整 -Xmn、-XX:SurvivorRatio 等参数设置来解决问题。
   2. 如果参数正常，但是 youngGC 频率还是太高，就需要使用 Jmap 和 MAT 对 dump 文件进行进一步排查了。

2. youngGC耗时过长：

   1. 耗时过长问题就要看 GC 日志里耗时耗在哪一块了。以 G1 日志为例，可以关注 Root Scanning、Object Copy、Ref Proc 等阶段。
      1. Ref Proc 耗时长，就要注意引用相关的对象。Root Scanning 耗时长，就要注意线程数、跨代引用。Object Copy 则需要关注对象生存周期。而且耗时分析它需要横向比较，就是和其他项目或者正常时间段的耗时比较。
      2. 比如说图中的 Root Scanning 和正常时间段比增长较多，那就是起的线程太多了。

3. Full GC

   1. G1 中更多的还是 mixedGC，但 mixedGC 可以和 youngGC 思路一样去排查。触发 Full GC 了一般都会有问题，G1 会退化使用 Serial 收集器来完成垃圾的清理工作，暂停时长达到秒级别，可以说是半跪了。

      1. 并发阶段失败：在并发标记阶段，MixGC 之前老年代就被填满了，那么这时候 G1 就会放弃标记周期。这种情况，可能就需要增加堆大小，或者调整并发标记线程数 -XX:ConcGCThreads。
      2. 晋升失败：在 GC 的时候没有足够的内存供存活/晋升对象使用，所以触发了 Full GC。这时候可以通过 -XX:G1ReservePercent 来增加预留内存百分比，减少 -XX:InitiatingHeapOccupancyPercent 来提前启动标记，-XX:ConcGCThreads 来增加标记线程数也是可以的。
      3. 大对象分配失败：大对象找不到合适的 Region 空间进行分配，就会进行 Full GC，这种情况下可以增大内存或者增大 -XX:G1HeapRegionSize。
      4. 程序主动执行 System.gc()：不要随便写就对了。
   ...
   2. 排查：
      ...
      ##### 3.5最重要的一点，要把fullgc发生时刻的dump文件和正常没有发生fullgc时间的dump文件都下载到本
      ```
...
1. 超时：
   ...
   1. **超时**：超时大体可以分为连接超时和读写超时。主要错误在应用层面

   2. TCP队列溢出：

      1. syns queue(半连接队列)，accept queue(全连接队列)，第三次握手的时候如果全连接队列满了，server 发送一个 RST 包给 client，表示废掉这个握手过程和这个连接。

      2. netstat 命令，执行 netstat -s | egrep "listen|LISTEN"：

         ![img](https://s5.51cto.com/oss/202005/18/8b9a26cb608c5d3c74a80782ebf8ff91.jpg-wh_600x-s_1258650631.jpg)

         overflowed 表示全连接队列溢出的次数，sockets dropped 表示半连接队列溢出的次数。

   3. RST异常：RST 包表示连接重置，用于关闭一些无用的连接，通常表示异常关闭，区别于四次挥手。

      1. 端口不存在：如果像不存在的端口发出建立连接 SYN 请求，那么服务端发现自己并没有这个端口则会直接返回一个 RST 报文，用于中断连接。
         ...
         一方长期未收到另一方的确认报文，在一定时间或重传次数后发出 RST 报文这种大多也和网络环境相关了，网络环境差可能会导致更多的 RST 报文。
      ...
      2. 使用 tcpdump 命令进行抓包，并使用 wireshark 进行简单分析了。
         1. tcpdump -i en0 tcp -w xxx.cap，en0 表示监听的网卡：
         2. 通过 wireshark 打开抓到的包，可能就能看到如下图所示，红色的就表示 RST 包了。

### 5. 死锁监测

[详细](https://zhuanlan.zhihu.com/p/136294283)

1. jps配合jstack
   1. jps查看java进程
   2. jstack 进程id  查看具体的堆栈信息
2. jconsole
3. VisualVM
...
[详细](https://blog.csdn.net/v_JULY_v/article/details/7382693)
...
1. 题型：
   1. TOP1：
      1. 分治+归并：分成n份，每份找出一个top1。最后对这些top1进行比较
   2. TOPK：
      1. 分治+归并：分成n份，每份找出topK，对这些topK进行比较。
      2. 小顶堆：构建包含k个元素的小顶堆。之后遍历每个元素，与小顶堆的堆顶元素比较，如果大于则更新堆。
   3. 找出不重复整数：
      1. 方法1：采用2-bit的Bit-Map（00不存在，01出现1次，10出现多次，11无意义）。共需`2^32*2bit=1GB`内存。取出所有对应位是01的整数
      2. 分治划分小文件，从小文件找出不重复的整数，排序，归并。
2. 方法
   1. 分布式处理之MapReduce：MapReduce是一种计算模型，简单的说就是将大批量的工作（数据）分解（MAP）执行，然后再将结果合并成最终结果（REDUCE）。这样做的好处是可以在任务被分解后，可以通过大量机器进行并行计算，减少整个操作的时间。但如果你要我再通俗点介绍，那么，说白了，Mapreduce的原理就是一个归并排序。
   2. 外排序
   3. Trie树
...
```
git reset --soft HEAD^  //HEAD^表示上一个版本，也可以写为HEAD~1，两次就是HEAD~2
	--soft 不删除工作空间的改动代码，撤销commit，不撤销git add file
	--head 删除工作空间的改动代码，撤销commit且撤销add
```
...
6. 应用场景：
   ...
   1. 多线程的线程池也是单例模式。
   2. 数据库的连接池。数据库连接池主要是节省打开或者关闭数据库连接所引起的效率损耗，用单例模式降低损耗。

7. [代码](https://www.jianshu.com/p/ccf9312f6a2e)：
...
3. 动态代理2：CGlib代理
   1. 特点：
      ...
      3. 代理的类不能为final
   2. 使用：
      1. 构造Interceptor对象继承MehtodInterceptor，重写intercept方法增强方法
      2. 构建Enhancer代理对象
      3. 使用代理对象的方法即可增强
...
* ConcreteSubject：具体主题（具体被观察者），该角色将有关状态存入具体观察者对象，在具体主题的内部状
* 态发生改变时，给所有注册过的观察者发送通知。
...
1.  优点：解除耦合，让耦合的双方都依赖于抽象，从而使得各自的变换都不会影响另一边的变换。
2. 缺点：在应用观察者模式时需要考虑一下开发效率和运行效率的问题，程序中包括一个被观察者、多个观察者，开发、调试等内容会比较复杂，而且在Java中消息的通知一般是顺序执行，那么一个观察者卡顿，会影响整体的执行效率，在这种情况下，一般会采用异步实现。

## 5. 策略模式

[详细](https://blog.csdn.net/jason0539/article/details/45007553?dist_request_id=&depth_1-)

实现某一个功能有多种算法或者策略，我们可以根据环境或者条件的不同选择不同的算法或者策略来完成该功能。

我们让算法和对象分开来，使得算法可以独立于使用它的客户而变化。
...
1. 定义：Bit-map就是用一个bit位来标记某个元素对应的Value， 而Key即是该元素。由于采用了Bit为单位来存储数据，因此在存储空间方面，可以大大节省。
2. 使用：以排序为例
   1. 开辟1Byte的空间，将`4, 7, 5, 3, 2`放进去，放进去后，空间为`0,0,1,1,1,1,0,1`
   2. 遍历Bit-Map，按顺序取出数字即可。

### 6. 字典树

1. 定义：Trie树，即字典树，又称单词查找树或键树，是一种树形结构，是一种哈希树的变种。典型应用是用于统计和排序大量的字符串（但不仅限于字符串），所以经常被搜索引擎系统用于文本词频统计。它的优点是：最大限度地减少无谓的字符串比较，查询效率比哈希表高。**Trie的核心思想是空间换时间。利用字符串的公共前缀来降低查询时间的开销以达到提高效率的目的。**
2. 性质：
...
面试官您好，我叫汪宇晨，本科毕业于安徽大学计算机科学与技术专业，目前研究生就读于东南大学计算机技术专业。在学校里目前主要跟导师在做一些深度学习方向的工作，目前主要在做的项目就是和国家电网合作的一个科研项目，非侵入时负荷分解。我之前有在之前有在华为实习过，现在暑期实习也正在阿里云的云网路部门这边进行实习，实习主要做的都是开发岗位的工作。这次的话，我还是主要是想找一个开发岗，因为相比于算法岗对数学，需要多看论文和做研究，并且具有很多不确定性，开发岗这种写好程序能有确定结果，能做出一定成果的工作我更加感兴趣。

## 全链路探测

因为我现在在阿里云里面主要是做一些网路管控层面的工作，我们组主要是在做一个iotcc，也就是物联设备上云的工作。像是我们日常生活中会有各种各样的物联设备，比如说手机的sim卡，共享单车里面的卡，还有像家庭里面的电表设备的卡，他们要连进阿里云，必须要经过一系列的网络设备，比如说pgw运营商的基站，隧道网关，NAT网管，私网VPC里面的交换机这些设备。那么在这个过程中，任何一个环节出问题，就会导致设备上云失败。然后我要做的就是定位问题所在地。比如说是公网到基站，还是基站到隧道网关等等环节。然后就是找出各段链路之间的告警，关联起来，找出主告警并发送通知，启动处理方案。

里面的主要思路就是分段探测，举个例子，比如使用gre网关去探测sim卡到gregw一端看是否通，用gre网关去探测gre网关到pgw基站这一段是否通。如果第一段是通的，第二段没通，说明中间这一段可能有问题。这时候再去监测gregw隧道网关到基站之间的心跳是否正常，如果心跳有少量P2级别的丢包，可能只是发生了隧道切换，不是连接出了问题。这样，这个主因定位就出来了。然后根据主因去通知租户和管理员这样子。这里面为了保证告警的实时性，我们用了实时计算flink去做。我们会把各段之间的告警日志写入SLS日志系统，就是阿里云内部的一个格式化日志系统，然后我们用flink去连接并且指定的logstore，同时启动多个job，将各段的信息提取统计出来，写入同一个logstore，最后用一个统一的job去分析这个logstore，自己根据预先构建的决策树，去定位出一个主告警。

项目的主要难点就在于初期的设计吧，就是根据数据库里面的数据记录，自己构建出一个整个链路的拓扑图，找到节点之间的对应关系。然后自己去构建一个决策树，理清楚告警之间的关系和措施，定位方案。最后实现到代码层面就主要是flink的具体应用了，比如说key的选择，状态的保存，然后如何去设计窗口，整合具体数据等等。然后目前构思的是，后期的话可能还会增加一些人工智能方面的东西，因为我本身研究生也做过机器学习相关的东西嘛，考虑能不能通过训练增加一些故障预测的功能这样子。

## LLT

LOW LEVER TEST， 通常由开发人员实施，是用来保障语句的覆盖率的。LLT是在开发阶段对系统进行尽早、尽快的验证，发现的缺陷也更容易定位和解决。我们编写测试用例时只需要引入jar包，无需其他任何额外环境，就可以进行测试的编写。

### 1. 普通开发者测试框架遇到的挑战

1. 中间件准备投入大，像是数据库，Redis，kafka这些环境准备，准备测试数据和运行实例，都需要大量的时间投入。这就导致在某些场景下，环境的数据的准备远超过测试本身的价值。
2. 测试的可读性，可维护性差。这个是主要是当我们写完开发者测试用例之后，最后展现在看板上的，只有测试覆盖率这些指标。但是用例本身的可读性和可维护性就相对较低。后续当我们的代码发生改变或者逻辑改变，我们想要维护以前的测试用例就出现了问题，可能会出现看不懂用例的内容呀，或者交接后后面的人无法有效快速的使用。

### 2. 问题解决方案

1. 提供了包括Redis，Kafka，数据库，HBase这些业务中可能用到的In-memory方案，实际测试的时候，我们不需要去准备开发环境了，框架已经把这些东西全部集成进来。在框架里利用mock或者反射的方式，将被测业务代码里，所有对生产环境的使用，全部转移到这些本地的内存化方案中。
2. 对于这些可维护性差的问题，我们框架提供了一种能力，让用例基本上全部以自然语言的方式书写，目的就是为了让任何人，包括看不懂代码的这部分人都能知道我们在测什么。我们只需写一些feature文件，对于每个用例用汉字或者英文这些自然语言标准要测试的场景，提供要测试的接口url，请求方式，请求参数，就可以执行用例并输出测试结果，检查是否和期望结果相匹配。唯一可能要写代码的部分的话，就是在写测试用例的过程中，开发者只需要限定测试的边界。比如说我们待测的微服务还去请求了其他微服务的接口，只需要在这里用mock的方式，构造一个期待的返回结果即可。
3. 整个框架的启动流程就是启动tomcat，然后tomcat去进行像spring，hibernate加载，还有各类中间件的初始化工作，本地微服务的启动。最后就是扫描指定目录下的feature文件，对文件进行解析，进行用例的自动执行，向指定url发送请求，最后输出测试结果，统计率这样。

### 3. 我的工作

1. 还有一个就是自己设计了一个RPC框架，就用netty和zookeeper作为网络通信框架和服务注册中心，然后用了一些随机，一致性哈希之类的负载均衡方式，实现了一些心跳机制，SPI机制，还自定义了一个通信协议，用来解决就是通信过程中可能发生的沾包，拆包问题。

2. 降低开发者测试的粒度。因为普通的用例都是针对微服务接口的，这里面包括了一些非常大型的微服务。然后需要设计了一种基于方法测试的方案，这部分是我来做的。

   当时觉得也没多难，就是想指定待测方法，参数这些，然后将请求转发到我自己的解析逻辑上，做一些json字符串的参数解析，用一些开源的Gson，fastjson这些工具进行映射，然后反射调用一下实际方法去做这样。结果这种方案完全不可行。我们平时手动去写一个类，然后去写变量呀，方法呀这些。但是公司这边为了解耦和方便修改的需求吧，这边的类和接口很多都是通过写yang文件或者yaml文件，然后配合一些内部插件去自动编译生成的。传参的方式也不一样，大多数场景都是通过建造者模式去生成参数的。这就导致我们实际传参需要传对应的builder对象，由框架通过反射去生成一个实际的参数。这就不能通过fastjson，Gson这些工具去实现自动转换，需要我来手动的对字符串参数进行数据解析，然后通过反射去赋值，调整和最终生成。这个过程中还会遇到像是用builder生成的参数里面还有yaml模型，yang模型这种参数，yaml，yang模型出现在map，list，set，array的泛型里，需要分类处理，手动的通过递归去构造builder和生成，还有类似很多复杂的场景。就是这种一层层循环嵌套，这个地方就用到了很多反射跟递归。 然后你也知道，json串是没有类型信息嘛，转换参数的时候可能遇到需要转换的成员变量参数类型为接口，就没有办法直接转换了，然后这里也是参考了一下这边leader的意见，就去做一个handler，手动指定接口与实现类的对应关系，然后在每一层解析的时候遇到接口参数都会尝试做一个类型替换。然后种种因素吧，这中间就涉及非常多的处理过程。

   然后为了简化使用，尽可能减少用户的配置，尽量吧所有流程能自动化的地方都自动化了，比如遇到重载方法的一些自动选择，参数构造，建造者模式的自动适配这些。

   此外还做了一些用例之间的数据隔离。理论上所有的测试用例应该是互不影响的，但是实际上前一个用例在数据库存储的数据会影响到第二个用例。为了支撑不同的场景，设计了一些数据隔离方案，可以开发者自动选择是否开启数据隔离，数据初始化，数据恢复等。原理的话大概就是根据需求，利用数据库在用例执行前去读取文件去做一个数据初始化，自动恢复的话就是每个用例都去做一个全量备份，执行完之后用全量备份文件进行回滚，类似于mysql的undo log这种。这里是用一个开源的工具dbutils去做的。

3. 另外一个就是Hbase内存方案的设计。Hbase有RowKey，列族，列限定符这些概念，这里就手动设计了一个内存方案的HBase，将所有的操作转换到一个嵌套的TreeMap上面，走我们的方法。比如key是一个行键，value是一个列族的列表，然后每个列族又包含map，key是列名，value是列值这样。然后所有的操作都会转换为对map的操作。因为测试嘛，数据量不大，所以可以实现的。最后用一个反射的方式，将实际使用的类替换成我们的代理类

## RPC框架

### 1. 介绍

RPC框架是一个远程过程调用框架，就是让客户端直接调用存在服务器上的某个方法或者对象，就像调用本地程序一样。这里我使用netty和zookeeper作为网络通信框架和服务注册中心，并使用了随机，一致性哈希的负载均衡方式，实现了一个心跳机制，还设计了自定义协议，用来解决可能会发生的沾包，拆包问题。

框架的启动流程是这样的

1. 对于服务器端：

首先我定义了一个RpcService注解。在Spring框架初始化bean的时候，会对标记了RpcService的注解的接口进行服务发布，就是在zookeeper上构造节点，以服务名为父节点，以具体的ip地址和端口为当前节点。

1. 对于客户端：

客户端这边我定义了一个RpcReference注解。当扫描到标准了这个注解的类的时候，框架会自动构造一个这个类的代理类，用这个代理类去替换掉原本的bean。这个代理类采用了JDK动态代理的方式，执行方法的时候，会根据方法名，方法参数，去构造我们的RpcRequest。然后通过指定的负载均衡方式，查找到对应的服务器节点，最后通过netty的handler pipeline去构造一个RpcMessage，向指定服务器发送请求，并返回对应方法执行的结果，收到响应。客户端再解析出来，作为执行的结果。

**序列化**：然后序列化采用了json和protobuf的方式，json的话根据实际情况，比如方法参数是否有接口这些情况，拼接实现类参数，然后服务端会进行额外解析这样。

为了**防止请求丢失**，在本地缓存了所有已发出但是没有接收到响应的请求，用requestId和具体的请求相对应。没有收到响应的话，就会在下次发送请求前进行重新发送，收到响应就从内部去除掉这条缓存。

**一致性hash**我这里先求所有服务器的md5码，然后哈希算法求出值。使用treemap保存。选择的时候就先tailMap找出所有大于等于hash值的服务器，然后firstEntry第一台服务器的地址。

**TCP沾包拆包**主要是通过自定义协议实现的。我们在网络中请求的传输，是通过序列化的方式传输的。我们自己定义了一个RpcMessage类完成的。这个类里面包含了我们的消息类型（心跳，消息），编码方式，压缩方式，请求id，请求data等。我们在网络传输的时候，会把它转换成一个二进制字节流。用encoder编码的时候，会去计算序列化和压缩之后消息体的长度，与消息头拼接求出总长度，然后写在消息头里，decode的时候根据这个长度进行解码，拆分出一个一个包。
...
**SPI机制**：就是我们先会去META-INF.extensions目录下去找要实现的接口的全限定类名对应的文件，找对应接口的实现类，加载到本地缓存里。这里主要就是进行一些字符串的解析，和反射创建实现类这些工作，下次取的时候，根据名字直接从本地缓存里进行取对应的类实例。比如说序列化方式，负载均衡方式的选取都是通过spi机制实现的。

holder主要保存每个接口的实现类名字和对应的实现类。cachedInstance就是实现类名字和对应的Holder。extension_instance就是一个实现类和对应的对象。

### 2. 详细

消息头：magic code, version, full length, messagetype, codec, compress, requestId
...
非侵入式负荷分解是我导师实验室跟国家电网合作的一个科研项目，主要是深度学习相关的。主要的内容是通过从所有电器总功率里分解出

每台电器的功率。比如说，我在家里使用电脑，台灯，空调， 冰箱，洗衣机这些电器。我只要采集到在某一时刻整个家庭所有电器的总功率，就能从其中知道你在用哪些电器，这些电器的功率是多少。

这里我主要是构建了一个基于混合输入的神经网络。因为平常使用的负荷分解都是基于毫秒级的，就是多少毫秒采集一次数据，这种开销比较大。我们要正式落地投入使用，肯定不能用如此开销巨大的方式，就采用了分钟级的数据，就是每分钟采集一次家庭总功率，在进行分解。为了提高精度，需要引入一些社会特征，比如说家庭人口数量，家庭人口的职业组成，家庭收入，家庭地域，温度，湿度，天气这些信息，因为这些信息其实在很大程度上会影响一个家庭的用电行为。所以就构造了一个基于混合输入的神经网络，分别对电器特征和社会特征做不通的神经网络处理。综合二者去进行一个负荷分解。

技术的话主要是用了深度学习，其实机器学习的隐马尔科夫，k-means，还有组合优化这些也都尝试过，最后还是深度学习的方式比较优秀。主要就是原始数据采集，数据去噪声，然后输入神经网络，输出结果这么一个流程。神经网络的实际主要采用了一种序列到点的架构。我们会为每台电器构造一个负荷分解模型，输入是一个时间段的电器功率数据，比如1-15分钟，每个分钟的时间数据构成一个序列。然后输出是中点，也就是第8分钟的数据作为电器的功率。这种方式可以让预测更加着重于中点时刻的数据，因为中点时刻与两边的数据相关性更大。具体结构的话就是用一维卷积层坐一些负荷特征提取，特征压缩，然后lstm去对时间序列做一个具体分析，最后通过一些全连接层，配合relu这些激活函数，算出最终的结果。



# HR面

面试官您好，我叫汪宇晨，本科毕业于安徽大学计算机科学与技术专业，目前研究生就读于东南大学计算机技术专业。在学校里目前主要跟导师在做一些深度学习方向的工作，目前主要在做的项目就是和国家电网合作的一个科研项目，非侵入时负荷分解。这次的话，我主要是想找一个开发岗位的工作，因为相比于算法岗对数学，需要多看论文和做研究，并且具有很多不确定性，开发岗这种写好程序能有确定结果，能做出一定成果的工作我更加感兴趣。之前有在华为实习过，做的主要也是开发岗的工作，体验下来我也感觉开发更适合我一点。

### 1. 职业规划

1. 从兴趣角度出发，我个人比较倾向于在技术岗发展。初期的话从开发开始，先不断提高自己的技术吧。深耕基础，提高基本代码能力。
2. 加强自己对底层的理解，不仅仅是完成需求，尝试是否能在现有的技术架构上不断优化。或者实行重构，提高项目性能。
3. 系统架构师是一个最终确认和评估系统需求，给出开发规范，搭建系统实现的核心构架，并澄清技术细节、扫清主要难点的技术人员。主要着眼于系统的“技术实现”。因此他/她应该是特定的开发平台、语言、工具的大师，对常见应用场景能给出最恰当的解决方案，同时要对所属的开发团队有足够的了解，能够评估自己的团队实现特定的功能需求需要的代价。 系统架构师负责设计系统整体架构，从需求到设计的每个细节都要考虑到，把握整个项目，使设计的项目尽量效率高，开发容易，维护方便，升级简单等。

### 2. 为什么选择腾讯

1. 同学推荐，包括个人对腾讯观感好点，包括工作氛围，工作环境
2. 阿里是零售事业群，盒马供应链，感觉偏边缘偏业务一点。腾讯是TEG数据平台，上次了解到偏技术，业务偏少。对个人成长更佳
3. 今天阿里反垄断罚款，感觉腾讯的发展前景更好一点。华为偏非互联网，综合型企业，还是想进互联网

### 2. 支持来深圳吗

1. 一线城市都可以，年轻想外出奋斗一下，父母支持
2. 

### 2. 最自豪的一件事

科研项目正式落地，南京全国首个试点地，也上了报道。大家还问这是谁做的

### 3. 最受挫折的一件事

工作忙，加班，项目忙，看论文，晚上熬夜，周末闭关。

工作初期：首次实习，不懂业务，不知道公司的项目流程

项目初期：不会深度学习，不会python，不懂nilm这个领域知识

### 3. 学习过程

深度学习领域专家吴恩达AndrwNG的视频，全英文看了两遍
...
复现 - 自己尝试idea（导师提供了很多idea）

### 3. 实习学到了什么

1. 解决问题的方式，自己解决 --- 查看文档 --- 请求他人协助
2. 工作流程，工作流，每人负责自己的模块
3. 互相帮助，有问题互相解决

### TEG

1. 腾讯技术工程事业群
2. TEG一直在腾讯内部承担着底层技术的重任，默默为公司提供全方位的运营解决方案和服务支持，为公司沉淀了大量技术储备。腾讯TEG支持着腾讯自己的业务，也通过腾讯云赋能各行各业，成为业内领先的基础架构云运营平台、云数据处理平台、互联网海量应用支撑平台，为亿级用户提供云计费服务和安全保障。
...
1. 阿里技术好，开源精神强，比如nacos，dubbo，druid，fastjson
2. 腾讯以C++为主，我目前是java岗，阿里更合适。  腾讯是TEG部门，相对边缘一点，感觉阿里的盒马部门现在发展很好
3. 最主要的原因是在深圳，我未来是打算在长三角，离家近一点。
...
难点：提高运行效率，上面有指令要尽快投入使用，这边功能还不齐全。很多人要用，且不会用。
