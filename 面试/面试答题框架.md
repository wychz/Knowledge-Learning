...
2. 目录：
   ...
   2. 软链接：相当于基于老文件A新建了个文件B，该文件B有新的inode，不过文件B内容是老文件A的路径。当老文件A删除后，文件B仍然存在，不过找不到指定文件了。
...
里面的主要思路就是分段探测，举个例子，比如使用sim卡去探测sim卡到gregw一端看是否通，用sim卡探测sim卡到pgw基站这一段是否通。如果第一段是通的，第二段没通，说明中间这一段可能有问题。这时候再去监测gregw隧道网关到基站之间的心跳是否正常，如果心跳有少量P2级别的丢包，可能只是发生了隧道切换，不是连接出了问题。这样，这个主因定位就出来了。然后根据主因去通知租户和管理员这样子。这里面为了保证告警的实时性，我们用了实时计算flink去做。我们会把各段之间的告警日志写入SLS日志系统，就是阿里云内部的一个格式化日志系统，然后我们用flink去连接并且指定的logstore，同时启动多个job，将各段的信息提取统计出来，写入同一个logstore，最后用一个统一的job去分析这个logstore，自己根据预先构建的决策树，去定位出一个主告警。
